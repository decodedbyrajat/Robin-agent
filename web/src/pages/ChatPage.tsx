 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (robin_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__ROBIN_SESSION_TOKEN__
  ...
  useEffect(() => {
    ...
    const token = window.__ROBIN_SESSION_TOKEN__;
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[robin-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="robin-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
...
declare global {
  interface Window {
    __ROBIN_SESSION_TOKEN__?: string;
  }
}
