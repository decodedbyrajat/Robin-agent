# Contributing to Robin Agent

Thank you for contributing to Robin Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `robin skills browse` (labeled "official") and install it with `robin skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Robin project](https://github.com/IfThenVoid/Robin). Users can install it with `robin skills install`.
...
```bash
git clone --recurse-submodules https://github.com/IfThenVoid/Robin.git
cd robin-agent
...
```bash
mkdir -p ~/.robin/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.robin/config.yaml
touch ~/.robin/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.robin/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/robin" ~/.local/bin/robin
...
├── cli.py                    # RobinCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (robin-cli, robin-telegram, etc.)
├── robin_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── robin_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.robin/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site source
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.robin/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.robin/`)

| Path | Purpose |
|------|---------|
| `~/.robin/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.robin/.env` | API keys and secrets |
| `~/.robin/auth.json` | OAuth credentials (Nous Portal) |
| `~/.robin/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.robin/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.robin/state.db` | SQLite session database |
| `~/.robin/sessions/` | JSON session logs |
| `~/.robin/cron/` | Scheduled job data |
| `~/.robin/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`robin_state.py`) with full-text search and unique session titles. JSON logs go to `~/.robin/sessions/`.
...
Four fields are supported under `metadata.robin`:
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `robin setup` or update `~/.robin/.env` locally.
...
- **Test it.** Run `robin --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.
...
Add to `_BUILTIN_SKINS` dict in `robin_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `robin_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Robin runs on Linux, macOS, and WSL2 on Windows. When writing code that touches the OS:
...
2. **Test manually**: Run `robin` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/IfThenVoid/Robin/issues)
- Include: OS, Python version, Robin version (`robin version`), full error traceback
...
- **Discord**: [github.com/IfThenVoid/Robin](https://github.com/IfThenVoid/Robin) — for questions, showcasing projects, and sharing skills
