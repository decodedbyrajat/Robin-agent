...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._robin_home", tmp_path), \
             patch("robin_constants.display_robin_home", return_value="~/.robin/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.robin/profiles/coder/config.yaml`"
