...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("robin/robin-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned robin/* and pr-* branches."""

    def test_prunes_orphaned_robin_branch(self, git_repo):
        """robin/robin-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "robin/robin-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "robin/robin-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "robin/robin-deadbeef" in result.stdout
        ...
        assert "robin/robin-deadbeef" in orphaned
        ...
        assert "robin/robin-deadbeef" not in result.stdout
