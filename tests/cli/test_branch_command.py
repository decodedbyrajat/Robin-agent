...
def session_db(tmp_path):
    ...
    os.environ["ROBIN_HOME"] = str(tmp_path / ".robin")
    os.makedirs(tmp_path / ".robin", exist_ok=True)
    from robin_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".robin" / "test_sessions.db")
    yield db
    db.close()
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from robin_cli.commands import COMMAND_REGISTRY
