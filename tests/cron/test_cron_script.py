...
- Path resolution (absolute, relative to ROBIN_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ROBIN_HOME."""
    robin_home = tmp_path / ".robin"
    robin_home.mkdir()
    (robin_home / "cron").mkdir()
    (robin_home / "cron" / "output").mkdir()
    (robin_home / "scripts").mkdir()
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ROBIN_DIR", robin_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", robin_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", robin_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", robin_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.robin/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ROBIN_SESSION_PLATFORM") is None
        assert os.environ.get("ROBIN_SESSION_CHAT_ID") is None
        assert os.environ.get("ROBIN_SESSION_CHAT_NAME") is None
