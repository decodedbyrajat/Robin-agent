...
import robin_cli.doctor as doctor
import robin_cli.gateway as gateway_cli
from robin_cli import doctor as doctor_mod
from robin_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "ROBIN_HOME", robin_home)
    monkeypatch.delenv("ROBIN_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("ROBIN_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_robin_home(self, tmp_path, provider=""):
        """Create a minimal ROBIN_HOME with config.yaml."""
        home = tmp_path / ".robin"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_robin_home(tmp_path, provider)
...
def test_run_doctor_accepts_robin_provider_ids_that_catalog_aliases(
