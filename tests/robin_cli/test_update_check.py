"""Tests for the update check mechanism in robin_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("robin_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ROBIN_HOME", str(tmp_path))
    with patch("robin_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import robin_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "robin_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when ROBIN_HOME has no git repo."""
    import robin_cli.banner as banner
    ...
    # Point ROBIN_HOME at a temp dir with no robin-agent/.git
    monkeypatch.setenv("ROBIN_HOME", str(tmp_path))
    with patch("robin_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from robin_cli.main import _invalidate_update_cache

    # Build a fake ~/.robin with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"ROBIN_HOME": str(default_home)}):
