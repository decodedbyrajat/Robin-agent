...
from robin_cli import setup as setup_mod
from robin_cli.setup import setup_model_provider
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import robin_cli.gateway as gateway_mod
...
def test_setup_gateway_in_container_shows_docker_guidance(monkeypatch, capsys):
    ...
    monkeypatch.setattr(robin_constants, "is_container", lambda: True)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("robin_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("robin_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "robin_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "robin_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_iftv_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("robin_cli.setup.managed_iftv_tools_enabled", lambda: True)
    monkeypatch.setenv("ROBIN_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("robin_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("robin_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
    ...
    from robin_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("robin_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("robin_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("robin_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "robin_cli.setup.get_iftv_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
...
def test_offer_launch_chat_relaunches_via_bin(monkeypatch):
    from robin_cli import setup as setup_mod
    from robin_cli import relaunch as relaunch_mod

    monkeypatch.setattr(setup_mod, "prompt_yes_no", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(relaunch_mod, "resolve_robin_bin", lambda: "/usr/local/bin/robin")
    ...
    assert exec_calls == [("/usr/local/bin/robin", ["/usr/local/bin/robin", "chat"])]
...
def test_offer_launch_chat_falls_back_to_module(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod, "prompt_yes_no", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(relaunch_mod, "resolve_robin_bin", lambda: None)
    ...
    assert exec_calls == [(sys.executable, [sys.executable, "-m", "robin_cli.main", "chat"])]
