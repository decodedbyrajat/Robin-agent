"""Comprehensive tests for robin_cli.profiles module.
...
from robin_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_robin_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and ROBIN_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.robin/profiles)
    * ROBIN_HOME  -> tmp_path/.robin  (so get_robin_home() agrees)
    * Creates the bare-minimum ~/.robin directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".robin"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("ROBIN_HOME", str(default_home))
    return tmp_path
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.robin must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".robin"
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("robin_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".robin" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_robin_home_returns_default(self, profile_env):
        # ROBIN_HOME points to tmp_path/.robin which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".robin" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom ROBIN_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("ROBIN_HOME", str(custom))
        # With Docker-aware roots, a custom ROBIN_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("robin")
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "robin.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["robin.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["robin.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "robin.ssi_health": {"workspace": "robin", "enabled": True}
            }
        }))
        ...
        assert "robin.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["robin.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["robin.heimdall"]["workspace"] == "robin"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "robin.ssi_health": {"aiPeer": "ssi_health"},
                "robin.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["robin.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["robin.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("robin-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".robin_history",
        ...
        excluded_prefixes = [
            "default/robin-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.robin_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_robin_profiles_function(self):
        script = generate_bash_completion()
        assert "_robin_profiles" in script

    def test_zsh_completion_has_robin_function(self):
        script = generate_zsh_completion()
        assert "_robin" in script
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_robin_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".robin" / "profiles"

    def test_default_robin_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_robin_home()
        assert home == tmp_path / ".robin"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (ROBIN_HOME outside ~/.robin), profiles go under ROBIN_HOME."""
    ...
    def test_default_robin_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_robin_home() returns ROBIN_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ROBIN_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (ROBIN_HOME under ~/.robin), profiles root is still ~/.robin/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under ROBIN_HOME."""
        from robin_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ROBIN_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under ROBIN_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ROBIN_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".robin" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from robin_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".robin"
