...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["robin", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(sys.modules, "robin_cli.plugins", types.SimpleNamespace(discover_plugins=lambda: None))
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["robin", "-z", "hello", "--toolsets", "web,terminal"])
    monkeypatch.setitem(sys.modules, "robin_cli.plugins", types.SimpleNamespace(discover_plugins=lambda: None))
    monkeypatch.setitem(sys.modules, "tools.mcp_tool", types.SimpleNamespace(discover_mcp_tools=lambda: None))
    monkeypatch.setattr(config_mod, "load_config", lambda: {})
    monkeypatch.setattr(config_mod, "get_container_exec_info", lambda: None)
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from robin_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "robin -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["ROBIN_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(model="nous/robin-test", provider="nous", toolsets="web, terminal")

    env = captured["env"]
    assert env["ROBIN_MODEL"] == "nous/robin-test"
    assert env["ROBIN_INFERENCE_MODEL"] == "nous/robin-test"
    assert env["ROBIN_TUI_PROVIDER"] == "nous"
    assert env["ROBIN_INFERENCE_PROVIDER"] == "nous"
    assert env["ROBIN_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["ROBIN_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("robin-tui-active-session-")
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import robin_cli.main as main_mod
    ...
    monkeypatch.setitem(
        sys.modules, "robin_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "robin --tui --resume 20260409_000001_abc123" in out
    assert 'robin --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "robin --tui --resume actual_session" in out
