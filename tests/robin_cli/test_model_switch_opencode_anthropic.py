...
``robin_cli.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``robin_cli.model_switch.switch_model`` was missing the
...
def _run_opencode_switch(
    ...
    with (
        patch("robin_cli.model_switch.resolve_alias", return_value=None),
        patch("robin_cli.model_switch.list_provider_models", return_value=[]),
        patch(
        ...
        patch(
            "robin_cli.models.validate_requested_model",
            return_value=_MOCK_VALIDATION,
        ),
        patch("robin_cli.model_switch.get_model_info", return_value=None),
        patch("robin_cli.model_switch.get_model_capabilities", return_value=None),
        patch("robin_cli.models.detect_provider_for_model", return_value=None),
    ):
        return switch_model(
            raw_input=raw_input,
...
class TestStaleConfigDefaultDoesNotWedgeResolver:
    ...
    def test_kimi_switch_keeps_v1_despite_claude_config_default(self, tmp_path, monkeypatch):
        ...
        # Re-import with the new ROBIN_HOME so config cache is fresh.
        import robin_cli.config as _cfg_mod
        importlib.reload(_cfg_mod)
        import robin_cli.runtime_provider as _rp_mod
        importlib.reload(_rp_mod)
        import robin_cli.model_switch as _ms_mod
