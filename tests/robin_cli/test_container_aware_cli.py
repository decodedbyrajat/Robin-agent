...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ROBIN_HOME with .container-mode file."""
    robin_home = tmp_path / ".robin"
    robin_home.mkdir()
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))
    monkeypatch.delenv("ROBIN_DEV", raising=False)

    container_mode = robin_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=robin-agent\n"
        "exec_user=robin\n"
        "robin_bin=/data/current-package/bin/robin\n"
    )
    return robin_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("robin_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "robin-agent"
    assert info["exec_user"] == "robin"
    assert info["robin_bin"] == "/data/current-package/bin/robin"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("robin_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_robin_dev(container_env, monkeypatch):
    """Returns None when ROBIN_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ROBIN_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_robin_dev_zero(container_env, monkeypatch):
    """ROBIN_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ROBIN_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        robin_home = Path(tmpdir) / ".robin"
        robin_home.mkdir()
        (robin_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("robin_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_robin_home": lambda: robin_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ROBIN_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=robin-custom\n"
        "exec_user=myuser\n"
        "robin_bin=/opt/robin/bin/robin\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "robin-custom"
    assert info["exec_user"] == "myuser"
    assert info["robin_bin"] == "/opt/robin/bin/robin"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from robin_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "robin"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "robin-agent" in cmd
    assert "/data/current-package/bin/robin" in cmd
    assert "chat" in cmd
