"""Tests for the `robin memory reset` CLI command.
...
- Profile-scoped reset (uses ROBIN_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ROBIN_HOME with memory files."""
    robin_home = tmp_path / ".robin"
    memories = robin_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nRobin repo is at ~/.robin/robin-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `robin memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        robin_home = tmp_path / ".robin"
        (robin_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ROBIN_HOME", str(robin_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ROBIN_HOME."""
        ...
        monkeypatch.setenv("ROBIN_HOME", str(profile_home))
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        robin_home = tmp_path / ".robin"
        robin_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("ROBIN_HOME", str(robin_home))

        # The memories dir won't exist; get_robin_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
