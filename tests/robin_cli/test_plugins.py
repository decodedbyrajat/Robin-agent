"""Tests for the Robin plugin system (robin_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<robin_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<robin_home>/plugins/``; we derive
    ``<robin_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ROBIN_HOME>/config.yaml.
        # Config is always read from ROBIN_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        robin_home_str = os.environ.get("ROBIN_HOME")
        if robin_home_str:
            robin_home = Path(robin_home_str)
        else:
            robin_home = base.parent
        robin_home.mkdir(parents=True, exist_ok=True)
        cfg_path = robin_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.robin/plugins/ are discovered."""
        plugins_dir = tmp_path / "robin_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("ROBIN_HOME", str(tmp_path / "robin_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ROBIN_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".robin" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        robin_home = tmp_path / "robin_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under robin_plugins.<name>."""
        plugins_dir = tmp_path / "robin_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ROBIN_HOME", str(tmp_path / "robin_test"))

        # Clean up any prior namespace module
        sys.modules.pop("robin_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "robin_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="robin_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import robin_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("robin_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        robin_home = tmp_path / "robin_test"
        plugins_dir = robin_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "robin_test" / "plugins"
        # Set ROBIN_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("robin_cli.plugins.PluginContext.dispatch_tool.__module__", "robin_cli.plugins"):
