...
existing Codex CLI tokens via `robin auth openai-codex`. The old
...
@pytest.fixture()
def robin_auth_only_env(tmp_path, monkeypatch):
...
def test_normal_path_still_works(robin_auth_only_env):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    (robin_home / "auth.json").write_text(
