"""Tests for the ``robin hooks`` CLI subcommand."""
...
from robin_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("ROBIN_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("ROBIN_ACCEPT_HOOKS", raising=False)
...
class TestHooksList:
    def test_empty_config(self, tmp_path):
        with patch("robin_cli.config.load_config", return_value={}):
    ...
    def test_shows_configured_and_consent_status(self, tmp_path):
        ...
        with patch("robin_cli.config.load_config", return_value=cfg):
            out = _run(SimpleNamespace(hooks_action="list"))
...
class TestHooksTest:
    def test_synthetic_payload_matches_production_shape(self, tmp_path):
        """`robin hooks test` must feed the script stdin in the same
        shape invoke_hook() would at runtime.  Prior to this fix,
        run_once bypassed _serialize_payload and the two paths diverged —
        scripts tested with `robin hooks test` saw different top-level
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `robin hooks doctor` used to run every
