"""Tests for Codex auth — tokens stored in Robin auth store (~/.robin/auth.json)."""
...
def _setup_robin_auth(robin_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    ...
    auth_file = robin_home / "auth.json"
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    robin_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (robin_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    robin_home = tmp_path / "robin"
    _setup_robin_auth(robin_home, access_token="")
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_robin_auth(robin_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))
    ...
    monkeypatch.setattr("robin_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    robin_home = tmp_path / "robin"
    _setup_robin_auth(robin_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    _save_codex_tokens({"access_token": "robin-at", "refresh_token": "robin-rt"})
    ...
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "robin-at"


def test_resolve_returns_robin_auth_store_source(tmp_path, monkeypatch):
    robin_home = tmp_path / "robin"
    _setup_robin_auth(robin_home)
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "robin-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("robin_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "robin_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "robin_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("robin_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("robin_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
