...
subparser, placing the flag BEFORE the subcommand (e.g. 'robin --yolo chat')
...
def _build_parser():
    """Build the robin argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets ROBIN_YOLO_MODE regardless of flag position.
    ...
    def test_yolo_before_chat_sets_env(self):
        ...
        assert os.environ.get("ROBIN_YOLO_MODE") == "1"
    ...
    def test_no_yolo_no_env(self):
        ...
        self._simulate_cmd_chat_yolo_check(args)
        assert os.environ.get("ROBIN_YOLO_MODE") is None
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `robin gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `robin <argv>` must exit 0 (help) rather than
