...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ROBIN_OVERLAYS) — bedrock
...
class TestProviderModelIdsBedrock:
    ...
    def test_falls_back_to_static_list_when_discovery_empty(self, monkeypatch):
        ...
        from robin_cli.models import _PROVIDER_MODELS, provider_model_ids
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ROBIN_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ROBIN_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from robin_cli.providers import ROBIN_OVERLAYS
        assert "bedrock" in ROBIN_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from robin_cli.providers import ROBIN_OVERLAYS
        assert ROBIN_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from robin_cli.providers import ROBIN_OVERLAYS
        assert ROBIN_OVERLAYS["bedrock"].auth_type == "aws_sdk"
