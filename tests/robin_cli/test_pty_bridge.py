"""Unit tests for robin_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf robin-ok"])
        try:
            output = _read_until(bridge, b"robin-ok")
            assert b"robin-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$ROBIN_PTY_TEST\""],
            env={**os.environ, "ROBIN_PTY_TEST": "pty-env-works"},
        )
        try:
