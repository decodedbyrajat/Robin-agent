"""Tests for robin_cli.tools_config platform tool persistence."""
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("robin_cli.tools_config.save_config"):
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/IfThenVoid/Robin/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (robin-cli, robin-telegram, etc.) must NOT
    ...
    ``robin tools``, but robin-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "robin-cli",  # <-- the culprit
    ...
    # robin-cli must NOT survive — it's a platform default, not an MCP server
    assert "robin-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_robin_telegram():
    """Same bug for Telegram — robin-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "robin-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "robin-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "robin-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_iftv_subscription_when_logged_in(monkeypatch):
    monkeypatch.setattr("robin_cli.tools_config.managed_iftv_tools_enabled", lambda: True)
    config = {"model": {"provider": "nous"}}

    monkeypatch.setattr(
        "robin_cli.iftv_subscription.get_nous_auth_status",
...
def test_visible_providers_hide_iftv_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.setattr("robin_cli.tools_config.managed_iftv_tools_enabled", lambda: False)
    config = {"model": {"provider": "nous"}}
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("robin_cli.tools_config._run_post_setup", lambda key: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("robin_cli.tools_config.managed_iftv_tools_enabled", lambda: True)
    monkeypatch.setattr("robin_cli.iftv_subscription.managed_iftv_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
    ...
    monkeypatch.setattr(
        "robin_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web", "image_gen", "tts", "browser"},
    )
    monkeypatch.setattr("robin_cli.tools_config.save_config", lambda config: None)
    ...
    # apply_iftv_managed_defaults per platform; a second iteration sees values
    # set by the first as "explicit" and skips them.
    monkeypatch.setattr(
        "robin_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "robin_cli.iftv_subscription.get_nous_auth_status",
    ...
    configured = []
    monkeypatch.setattr(
        "robin_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from robin_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """robin-gateway includes list should cover all messaging platforms."""
        from robin_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["robin-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from robin_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from robin_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/IfThenVoid/Robin/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from robin_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from robin_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from robin_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("robin_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("robin_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("robin_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`robin tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["robin-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "robin-_test_platform"},
    }

    with mock_patch("robin_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'robin-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `robin tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from robin_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `robin tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from robin_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `robin tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from robin_cli.tools_config import _get_effective_configurable_toolsets
