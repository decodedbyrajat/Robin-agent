"""Tests for the top-level `./robin` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./robin` should use `robin_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "robin"
    called = []

    fake_main_module = types.ModuleType("robin_cli.main")

    def fake_main():
        called.append("robin_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "robin_cli.main", fake_main_module)
