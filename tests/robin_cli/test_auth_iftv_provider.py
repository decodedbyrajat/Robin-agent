...
from robin_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from robin_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ROBIN_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_robin_ca_bundle_env_falls_back(self, monkeypatch):
        from robin_cli.auth import _resolve_verify

        monkeypatch.setenv("ROBIN_CA_BUNDLE", "/nonexistent/robin-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (robin_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from robin_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    robin_home = tmp_path / "robin"
    _setup_nous_auth(robin_home, access_token="at-123")
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    robin_home = tmp_path / "robin"
    _setup_nous_auth(robin_home, access_token="at-fresh")
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("robin_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    robin_home = tmp_path / "robin"
    _setup_nous_auth(robin_home, refresh_token="refresh-old")
    monkeypatch.setenv("ROBIN_HOME", str(robin_home))
    ...
    monkeypatch.setattr("robin_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("robin_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `robin model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        auth_path = robin_home / "auth.json"
        auth_path.write_text(json.dumps({
            "version": 1,
            "active_provider": "openrouter",
            "providers": {"openrouter": {"api_key": "sk-or-fake"}},
        }))
        return robin_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import robin_cli.auth as auth_mod
        import robin_cli.models as models_mod
        import robin_cli.iftv_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from robin_cli.auth import PROVIDER_REGISTRY, _login_nous

        robin_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `robin auth add nous`
    ...
    """
    from robin_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Robin is not logged into Nous Portal").
    ...
    """
    from robin_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
    ...
    # doesn't raise "Robin is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `robin auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    """
    from robin_cli.auth import _refresh_access_token
    ...
    assert "robin auth add nous" in message.lower()
