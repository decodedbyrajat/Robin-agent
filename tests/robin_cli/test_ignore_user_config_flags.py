"""Tests for --ignore-user-config and --ignore-rules flags on `robin chat`.
...
* ``--ignore-user-config`` → skip ``~/.robin/config.yaml`` in ``load_cli_config()``
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("ROBIN_IGNORE_USER_CONFIG", "ROBIN_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``ROBIN_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<robin_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._robin_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_robin_home", tmp_path)
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("ROBIN_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting ROBIN_IGNORE_RULES=1 flips RobinCLI.ignore_rules True."""
        monkeypatch.setenv("ROBIN_IGNORE_RULES", "1")
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("ROBIN_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("ROBIN_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.RobinCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``robin_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("ROBIN_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("ROBIN_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("ROBIN_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("ROBIN_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("ROBIN_IGNORE_USER_CONFIG") == "1"
        assert "ROBIN_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "ROBIN_IGNORE_USER_CONFIG" not in os.environ
        assert "ROBIN_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real robin parser accepts both flags."""
        ...
        import inspect
        import robin_cli.main as hm
        src = inspect.getsource(hm)
        assert "ROBIN_IGNORE_USER_CONFIG" in src
        assert "ROBIN_IGNORE_RULES" in src
