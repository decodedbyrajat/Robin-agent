...
from acp_adapter.server import RobinACPAgent, ROBIN_VERSION
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "robin-agent"
        assert resp.agent_info.version == ROBIN_VERSION
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        with patch(
            "robin_cli.models.curated_models_for_provider",
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("robin_cli.config.load_config", lambda: {
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert ROBIN_VERSION in result
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["robin-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["robin-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["robin-acp", "mcp-srv"]
