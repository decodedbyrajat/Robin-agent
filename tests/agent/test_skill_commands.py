...
"""
    ...
    def test_gateway_still_loads_skill_but_returns_setup_guidance(
        ...
        with patch.dict(
            os.environ, {"ROBIN_SESSION_PLATFORM": "telegram"}, clear=False
        ):
            with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
                _make_skill(
                    tmp_path,
...
class TestTemplateVarSubstitution:
    """``${ROBIN_SKILL_DIR}`` and ``${ROBIN_SESSION_ID}`` in SKILL.md body
    ...
    def test_substitutes_skill_dir(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run: node ${ROBIN_SKILL_DIR}/scripts/foo.js",
            )
            scan_skill_commands()
        ...
        assert "${ROBIN_SKILL_DIR}" not in msg.split("[Skill directory:")[0]
    ...
    def test_leaves_session_id_token_when_missing(self, tmp_path):
        ...
        assert "Session: ${ROBIN_SESSION_ID}" in msg
    ...
    def test_disable_template_vars_via_config(self, tmp_path):
        ...
        assert "${ROBIN_SKILL_DIR}/scripts/foo.js" in msg
