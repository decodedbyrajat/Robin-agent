...
def curator_env(tmp_path, monkeypatch):
    """Isolated ROBIN_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".robin"
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    """robin curator unpin must refuse bundled/hub skills too (matches pin)."""
    from robin_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `robin model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from robin_cli.main import _AUX_TASKS
    from robin_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. robin_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. robin_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
