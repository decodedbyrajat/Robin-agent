"""Tests for agent/iftv_rate_guard.py — cross-session Nous Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    robin_home = str(tmp_path / ".robin")
    os.makedirs(robin_home, exist_ok=True)
    monkeypatch.setenv("ROBIN_HOME", robin_home)
...
class TestRecordNousRateLimit:
    ...
    def test_records_with_header_reset(self, rate_guard_env):
        from agent.iftv_rate_guard import record_iftv_rate_limit, _state_path

        headers = {"x-ratelimit-reset-requests-1h": "1800"}
        record_iftv_rate_limit(headers=headers)
    ...
    def test_falls_back_to_default_cooldown(self, rate_guard_env):
        ...
        record_iftv_rate_limit(headers=None)
    ...
    def test_custom_default_cooldown(self, rate_guard_env):
        ...
        record_iftv_rate_limit(headers=None, default_cooldown=120.0)
    ...
    def test_creates_directory_if_missing(self, rate_guard_env):
        ...
        record_iftv_rate_limit(headers={"retry-after": "10"})
...
class TestNousRateLimitRemaining:
    ...
    def test_returns_none_when_no_file(self, rate_guard_env):
        from agent.iftv_rate_guard import iftv_rate_limit_remaining

        assert iftv_rate_limit_remaining() is None

    def test_returns_remaining_seconds_when_active(self, rate_guard_env):
        from agent.iftv_rate_guard import record_iftv_rate_limit, iftv_rate_limit_remaining

        record_iftv_rate_limit(headers={"x-ratelimit-reset-requests-1h": "600"})
        remaining = iftv_rate_limit_remaining()
    ...
    def test_returns_none_when_expired(self, rate_guard_env):
        from agent.iftv_rate_guard import iftv_rate_limit_remaining, _state_path
...
class TestClearNousRateLimit:
    ...
    def test_clears_existing_file(self, rate_guard_env):
        ...
        record_iftv_rate_limit(headers={"retry-after": "600"})
        assert iftv_rate_limit_remaining() is not None
...
class TestFormatRemaining:
    ...
    def test_seconds(self):
        from agent.iftv_rate_guard import format_remaining
...
class TestParseResetSeconds:
    ...
    def test_case_insensitive_headers(self, rate_guard_env):
        from agent.iftv_rate_guard import _parse_reset_seconds
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_nous_skips_when_rate_limited(self, rate_guard_env, monkeypatch):
        from agent.iftv_rate_guard import record_iftv_rate_limit
...
class TestIsGenuineNousRateLimit:
    ...
    Nous Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Robin).
    ...
    def test_exhausted_hourly_bucket_in_429_headers_is_genuine(self):
        from agent.iftv_rate_guard import is_genuine_iftv_rate_limit
        ...
        assert is_genuine_iftv_rate_limit(headers=headers) is True
    ...
    def test_healthy_headers_on_429_are_upstream_capacity(self):
        ...
        assert is_genuine_iftv_rate_limit(headers=headers) is False

    def test_bare_429_with_no_headers_is_upstream(self):
        from agent.iftv_rate_guard import is_genuine_iftv_rate_limit

        assert is_genuine_iftv_rate_limit(headers=None) is False
        assert is_genuine_iftv_rate_limit(headers={}) is False
        assert is_genuine_iftv_rate_limit(
