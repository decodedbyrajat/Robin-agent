...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `robin cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # robin_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.robin/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "robin_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    ...
    # All-lowercase matches → lowercase ``robin``; this preserves the
    # real filesystem path ``~/.robin`` (Robin home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "robin should always respond concisely"
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("clawdbot prefers tabs") == "robin prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Robin was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "robin uses Python"
...
def test_rebrand_text_preserves_filesystem_path_casing():
    ...
    rewrite to lowercase ``.robin`` (the real Robin home), not the broken
    ``.Robin``.
    ...
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.robin/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .robin directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.robin'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "robin config path: ~/.robin/"
