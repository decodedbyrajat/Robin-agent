"""
Tests for timezone support (robin_time module + integration points).
...
def _reset_robin_time_cache():
    """Reset the robin_time module cache (replacement for removed reset_cache)."""
    robin_time._cached_tz = None
    robin_time._cached_tz_name = None
    robin_time._cache_resolved = False


# =========================================================================
# robin_time.now() — core helper
# =========================================================================
...
class TestRobinTimeNow:
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["ROBIN_TIMEZONE"] = "Asia/Kolkata"
        result = robin_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["ROBIN_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="robin_time"):
            result = robin_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ROBIN_TIMEZONE"] = "Europe/London"
        tz = robin_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["ROBIN_TIMEZONE"] = "Not/A/Timezone"
        tz = robin_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ROBIN_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ROBIN_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ROBIN_TIMEZONE=" + os.environ.get("ROBIN_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ROBIN_TIMEZONE=NOT_SET" in result["output"], (
            "ROBIN_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ROBIN_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ROBIN_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=robin_tz) which shifted
    ...
    def test_ensure_aware_normalizes_aware_to_robin_tz(self):
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        # of the naive timestamp exceeds _robin_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ROBIN_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_robin_time_cache()
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ROBIN_TIMEZONE"] = "US/Eastern"
        _reset_robin_time_cache()
