...
class TestToolsetConsistency:
    ...
    def test_robin_platforms_share_core_tools(self):
        """All robin-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on robin-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["robin-cli", "robin-telegram", "robin-discord", "robin-whatsapp", "robin-slack", "robin-signal", "robin-homeassistant"]
