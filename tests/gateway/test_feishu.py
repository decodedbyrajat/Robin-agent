...
class TestAdapterBehavior(unittest.TestCase):
    ...
    @patch.dict(
        os.environ,
        {
            "FEISHU_BOT_OPEN_ID": "ou_robin",
            "FEISHU_BOT_USER_ID": "u_robin",
        },
        clear=True,
    )
    ...
    def test_self_bot_sender_is_treated_as_self_sent_message(self):
        ...
        by_open_id = SimpleNamespace(
            sender=SimpleNamespace(
                sender_type="bot",
                sender_id=SimpleNamespace(open_id="ou_robin", user_id="u_other"),
            )
        )
        by_user_id = SimpleNamespace(
            sender=SimpleNamespace(
                sender_type="app",
                sender_id=SimpleNamespace(open_id="ou_other", user_id="u_robin"),
            )
        )
    ...
    @patch.dict(
        os.environ,
        {
            "ROBIN_FEISHU_TEXT_BATCH_MAX_MESSAGES": "2",
        },
        clear=True,
    )
    ...
    def test_dedup_state_persists_across_adapter_restart(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"ROBIN_HOME": temp_home}, clear=False):
    ...
    def test_send_splits_fenced_code_blocks_into_separate_post_rows(self):
        ...
        content = (
            "确认已入库 ✓\n"
            "文件路径：`/root/.robin/profiles/agent_cto/cron/jobs.json`\n"
            "**解码后的内容：**\n"
            "```json\n"
            '{"cron": "list"}\n'
            "```\n"
...
class TestHydrateBotIdentity(unittest.TestCase):
    ...
    def test_hydration_populates_open_id_from_bot_info(self):
        ...
        self.assertEqual(adapter._bot_open_id, "ou_robin_hydrated")
    ...
    def test_hydrated_open_id_enables_self_send_filter(self):
        ...
        adapter._client = Mock()
        payload = json.dumps(
            {"code": 0, "bot": {"bot_name": "Robin", "open_id": "ou_robin"}}
        ).encode("utf-8")
        adapter._client.request = Mock(return_value=SimpleNamespace(raw=SimpleNamespace(content=payload)))
        ...
        self_event = SimpleNamespace(
            sender=SimpleNamespace(
                sender_type="bot",
                sender_id=SimpleNamespace(open_id="ou_robin", user_id=""),
            )
        )
