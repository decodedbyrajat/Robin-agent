...
async def test_auto_registers_plugin_commands_for_discord(adapter):
    ...
    with patch(
        "robin_cli.plugins.get_plugin_commands",
        return_value={
            "metricas": {
...
@pytest.mark.asyncio
async def test_auto_create_thread_falls_back_to_robin_when_only_mentions(adapter):
...
def test_discord_auto_thread_config_bridge(monkeypatch, tmp_path):
    ...
    robin_dir.mkdir()
    config_path = robin_dir / "config.yaml"
    config_path.write_text(yaml.dump({
        "discord": {"auto_thread": True},
    }))

    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    monkeypatch.setenv("ROBIN_HOME", str(robin_dir))
...
def test_register_skill_command_is_flat_not_nested(adapter):
    ...
    with patch(
        "robin_cli.commands.discord_skill_commands_by_category",
