...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.robin/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("robin_cli.config.load_config", return_value={"skills": {}}):
    ...
    def test_guard_flag_reads_config_when_set(self):
        ...
        with patch("robin_cli.config.load_config",
    ...
    def test_guard_flag_handles_config_error(self):
        ...
        with patch("robin_cli.config.load_config", side_effect=RuntimeError("boom")):
            assert _guard_agent_created_enabled() is False
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.robin/skills/ as a
    workaround.
    """
...
# The user unpins via `robin curator unpin <name>`.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_edit_refuses_pinned(self, tmp_path):
        ...
        assert "robin curator unpin my-skill" in result["error"]
