...
Validates that:
1. The installed atroposlib API still matches Robin's expectations
2. Robin's parser registry remains compatible with ManagedServer parsing
3. RobinAgentBaseEnv wires the selected parser into ServerManager correctly

These tests verify the contract between robin-agent's environments/ code
...
class TestManagedServerAPI:
    """Test that ManagedServer's API matches what robin-agent expects."""
...
class TestParserCompatibility:
    """Test that robin-agent's parsers match ManagedServer's expectations."""
...
class TestBaseEnvCompatibility:
    """Test that robin_base_env.py's tool-parser wiring matches the current API."""

    def test_robin_base_env_sets_server_manager_tool_parser(self):
        """Robin wires parser selection through ServerManager.tool_parser."""
        import ast

        base_env_path = Path(__file__).parent.parent.parent / "environments" / "robin_base_env.py"
        ...
        assert found_assignment, (
            "robin_base_env.py should set self.server.tool_parser from config.tool_call_parser"
        )

    def test_robin_base_env_uses_config_tool_call_parser(self):
        """Verify robin_base_env uses the config field rather than a local parser instance."""
        base_env_path = Path(__file__).parent.parent.parent / "environments" / "robin_base_env.py"
        source = base_env_path.read_text()
