...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        robin_home.mkdir()
        (robin_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"ROBIN_HOME": str(robin_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(robin_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.robin/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        robin_home = tmp_path / ".robin"
        robin_home.mkdir()
        (robin_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        robin_home = tmp_path / ".robin"
        robin_home.mkdir()
        (robin_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        robin_home = tmp_path / ".robin"
        robin_home.mkdir()
        (robin_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.robin/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        robin_home = tmp_path / ".robin"
        (robin_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ROBIN_HOME": str(robin_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.robin")

        assert mounts[0]["container_path"] == "/home/user/.robin/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.robin/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.robin/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ROBIN_HOME."""
        monkeypatch.setenv("ROBIN_HOME", str(tmp_path / ".robin"))
        (tmp_path / ".robin").mkdir()

        # Create a sensitive file one level above robin_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside robin_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ROBIN_HOME must still be registered."""
        robin_home = tmp_path / ".robin"
        robin_home.mkdir()
        monkeypatch.setenv("ROBIN_HOME", str(robin_home))
        (robin_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_robin_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ROBIN_HOME must be allowed."""
        robin_home = tmp_path / ".robin"
        robin_home.mkdir()
        subdir = robin_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ROBIN_HOME pointing outside must be rejected."""
        robin_home = tmp_path / ".robin"
        robin_home.mkdir()
        monkeypatch.setenv("ROBIN_HOME", str(robin_home))

        # Create a sensitive file outside robin_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside robin_home pointing outside
        symlink = robin_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ROBIN_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, robin_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ROBIN_HOME."""
        robin_home = tmp_path / ".robin"
        robin_home.mkdir()
        monkeypatch.setenv("ROBIN_HOME", str(robin_home))
        ...
        sensitive.write_text("{}")
        self._write_config(robin_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(robin_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ROBIN_HOME via config must still mount."""
        robin_home = tmp_path / ".robin"
        robin_home.mkdir()
        monkeypatch.setenv("ROBIN_HOME", str(robin_home))

        (robin_home / "oauth.json").write_text("{}")
        self._write_config(robin_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        robin_home = tmp_path / ".robin"
        robin_home.mkdir()
        (robin_home / "cache" / "documents").mkdir(parents=True)
        (robin_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("ROBIN_HOME", str(robin_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.robin/cache/documents" in paths
        assert "/root/.robin/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.robin/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        robin_home = tmp_path / ".robin"
        robin_home.mkdir()
        # Use legacy dir name — get_robin_dir prefers old if it exists
        (robin_home / "document_cache").mkdir()
        (robin_home / "image_cache").mkdir()
        monkeypatch.setenv("ROBIN_HOME", str(robin_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(robin_home / "document_cache") in host_paths
        assert str(robin_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.robin/cache/documents" in container_paths
        assert "/root/.robin/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        robin_home = tmp_path / ".robin"
        doc_dir = robin_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        robin_home = tmp_path / ".robin"
        ss_dir = robin_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.robin/cache/screenshots/session_abc/screen1.png"
