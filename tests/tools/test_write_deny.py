...
class TestWriteDenyExactPaths:
    ...
    def test_robin_env(self):
        # ``.env`` under the active ROBIN_HOME (profile-aware, not just
        # ``~/.robin``) must be write-denied. The hermetic test conftest
        # points ROBIN_HOME at a tempdir — resolve via get_robin_home()
        # to match the denylist.
        from robin_constants import get_robin_home
        path = str(get_robin_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_robin_config_not_env(self):
        path = os.path.join(str(Path.home()), ".robin", "config.yaml")
