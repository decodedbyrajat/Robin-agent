...
def _clean_state():
    ...
    saved = {}
    for k in ("ROBIN_INTERACTIVE", "ROBIN_GATEWAY_SESSION", "ROBIN_EXEC_ASK", "ROBIN_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ROBIN_INTERACTIVE"] = "1"
        session_key = os.getenv("ROBIN_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ROBIN_INTERACTIVE or ROBIN_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
