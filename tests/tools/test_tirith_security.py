...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._robin_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_robin_bin(self):
        """After _INSTALL_FAILED, manual install in ROBIN_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        robin_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(robin_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(robin_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._robin_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == robin_bin
            assert _tirith_mod._resolved_path == robin_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# ROBIN_HOME isolation
# ---------------------------------------------------------------------------

class TestRobinHomeIsolation:
    def test_robin_bin_dir_respects_robin_home(self):
        """_robin_bin_dir must use ROBIN_HOME, not hardcoded ~/.robin."""
        from tools.tirith_security import _robin_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ROBIN_HOME": tmpdir}):
            result = _robin_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_robin_home(self):
        """_failure_marker_path must use ROBIN_HOME, not hardcoded ~/.robin."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ROBIN_HOME": "/custom/robin"}):
            result = _failure_marker_path()
        assert result == "/custom/robin/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ROBIN_HOME; verify it's active."""
        robin_home = os.getenv("ROBIN_HOME")
        assert robin_home is not None, "ROBIN_HOME should be set by conftest"
        assert "robin_test" in robin_home, "Should point to test temp dir"

    def test_get_robin_home_fallback(self):
        """Without ROBIN_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_robin_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ROBIN_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".robin")
            result = _get_robin_home()
