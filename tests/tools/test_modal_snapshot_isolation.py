...
def _restore_tool_modules():
    original_robin_home = os.environ.get("ROBIN_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_robin_home is None:
            os.environ.pop("ROBIN_HOME", None)
        else:
            os.environ["ROBIN_HOME"] = original_robin_home
        _reset_modules(("tools", "robin_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    robin_cli = types.ModuleType("robin_cli")
    robin_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["robin_cli"] = robin_cli
    robin_home = tmp_path / "robin-home"
    os.environ["ROBIN_HOME"] = str(robin_home)
    sys.modules["robin_cli.config"] = types.SimpleNamespace(
        get_robin_home=lambda: robin_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="robin-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": robin_home / "modal_snapshots.json",
