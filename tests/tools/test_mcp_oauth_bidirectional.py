...
@pytest.mark.asyncio
async def test_robin_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import _ROBIN_PROVIDER_CLS, reset_manager_for_tests

    assert _ROBIN_PROVIDER_CLS is not None, "SDK OAuth types must be available"
...
@pytest.mark.asyncio
async def test_robin_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
