...
@pytest.fixture(autouse=True)
def _enable_managed_iftv_tools(monkeypatch):
    """Ensure managed_iftv_tools_enabled() returns True even after module reloads.
    ...
    robin_cli.auth and robin_cli.models — so the function body returns True.
    """
    monkeypatch.setattr("robin_cli.auth.get_nous_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr("robin_cli.models.check_nous_free_tier", lambda: False)
