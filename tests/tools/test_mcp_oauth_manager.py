...
def test_robin_provider_subclass_exists():
    """RobinMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ROBIN_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ROBIN_PROVIDER_CLS is not None
    assert issubclass(_ROBIN_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_robin_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns RobinMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ROBIN_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ROBIN_HOME", str(tmp_path))
    ...
    assert _ROBIN_PROVIDER_CLS is not None
    assert isinstance(provider, _ROBIN_PROVIDER_CLS)
    assert provider._robin_server_name == "srv"
