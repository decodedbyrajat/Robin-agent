...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._robin_home / "logs" / "errors.log").resolve()
...
class TestBuildSystemPrompt:
    ...
    def test_includes_iftv_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_iftv_subscription_prompt", lambda tool_names: "NOUS SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_blocked_returns_error_and_skips_execution(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "robin_cli.plugins.get_pre_tool_call_block_message",
...
class TestRunConversation:
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch("robin_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
...
class TestNousCredentialRefresh:
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "robin_cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )
