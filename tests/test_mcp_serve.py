...
def _isolate_robin_home(tmp_path, monkeypatch):
    """Redirect ROBIN_HOME to a temp directory."""
    monkeypatch.setenv("ROBIN_HOME", str(tmp_path))
    try:
        import robin_constants
        monkeypatch.setattr(robin_constants, "get_robin_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking robin_state schema."""
