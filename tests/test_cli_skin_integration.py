...
from robin_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("robin_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_robin_branding(self):
        set_active_skin("default")
        ...
        assert "NOUS ROBIN" in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_robin(self):
        ...
        assert "NOUS ROBIN" not in banner
