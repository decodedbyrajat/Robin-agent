"""Shared fixtures for the robin-agent test suite.
...
2. **Isolated ROBIN_HOME.** ROBIN_HOME points to a per-test tempdir so
   code reading ``~/.robin/*`` via ``get_robin_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".robin"`` instead of the canonical
   ``get_robin_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ROBIN_SESSION_* inheritance** — the agent's current gateway
...
# ROBIN_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ROBIN_BEHAVIORAL_VARS = frozenset({
    "ROBIN_YOLO_MODE",
    "ROBIN_INTERACTIVE",
    "ROBIN_QUIET",
    "ROBIN_TOOL_PROGRESS",
    ...
    "ROBIN_SESSION_CHAT_ID",
    "ROBIN_SESSION_CHAT_NAME",
    "ROBIN_SESSION_THREAD_ID",
    "ROBIN_SESSION_SOURCE",
    "ROBIN_SESSION_KEY",
    "ROBIN_GATEWAY_SESSION",
    "ROBIN_PLATFORM",
    "ROBIN_MODEL",
    "ROBIN_INFERENCE_MODEL",
    ...
    "ROBIN_TIMEZONE",
    "ROBIN_REDACT_SECRETS",
    "ROBIN_BACKGROUND_NOTIFICATIONS",
    "ROBIN_EXEC_ASK",
    "ROBIN_HOME_MODE",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ROBIN_HOME to per-test tempdirs so code that
    reads ``~/.robin/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ROBIN_* vars that could change test semantics.
    for name in _ROBIN_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ROBIN_HOME to a per-test tempdir. Code that reads
    #    ``~/.robin/*`` via ``get_robin_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.robin/*`` via
    #    ``Path.home() / ".robin"`` instead of ``get_robin_home()``
    #    is a bug to fix at the callsite.
    fake_robin_home = tmp_path / "robin_test"
    fake_robin_home.mkdir()
    (fake_robin_home / "sessions").mkdir()
    (fake_robin_home / "cron").mkdir()
    (fake_robin_home / "memories").mkdir()
    (fake_robin_home / "skills").mkdir()
    monkeypatch.setenv("ROBIN_HOME", str(fake_robin_home))
    ...
    #    ~/.robin/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import robin_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_robin_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "robin_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal robin config dict suitable for unit tests."""
    return {
