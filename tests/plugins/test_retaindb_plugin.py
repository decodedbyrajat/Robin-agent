...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure ROBIN_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def _make_provider(self, tmp_path, monkeypatch, api_key="rdb-test-key"):
        ...
        (tmp_path / ".robin").mkdir(exist_ok=True)
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", robin_home=str(tmp_path / ".robin"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", robin_home=profile_home)
        assert p._client.project == "robin-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".robin" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        robin_home = tmp_path / ".robin"
        robin_home.mkdir(exist_ok=True)
        monkeypatch.setenv("ROBIN_HOME", str(robin_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", robin_home=str(robin_home))
        return p
