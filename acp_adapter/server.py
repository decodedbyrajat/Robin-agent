"""ACP agent server — exposes Robin Agent via the Agent Client Protocol."""
...
try:
    from robin_cli import __version__ as ROBIN_VERSION
except Exception:
    ROBIN_VERSION = "0.0.0"
...
class RobinACPAgent(acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from robin_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from robin_cli.models import detect_provider_for_model, parse_model_input
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["robin-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Robin using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="robin-agent", version=ROBIN_VERSION),
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        restoring server-side state makes Robin remember context, but leaves the
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # not the executor's. Also set ROBIN_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue ROBIN_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind ROBIN_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("ROBIN_INTERACTIVE")
            ...
            try:
                ...
                # Restore ROBIN_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("ROBIN_INTERACTIVE", None)
                else:
                    os.environ["ROBIN_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (ROBIN_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Robin Agent v{ROBIN_VERSION}"
