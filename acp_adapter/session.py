...
Sessions are persisted to the shared SessionDB (``~/.robin/state.db``) so they
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["robin-acp"]):
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            ...
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.robin/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ROBIN_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_robin_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=robin_home / "state.db")
            return self._db_instance
        except Exception:
