...
TOOLSETS = {
    ...
    "robin-cli": {
        ...
        "tools": _ROBIN_CORE_TOOLS,
        "includes": []
    },

    "robin-cron": {
        # Mirrors robin-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `robin tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant, rl) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as robin-cli; gated by `robin tools`",
        "tools": _ROBIN_CORE_TOOLS,
        "includes": []
    },
    ...
    "robin-gateway": {
        ...
        "tools": [],
        "includes": ["robin-telegram", "robin-discord", "robin-whatsapp", "robin-slack", "robin-signal", "robin-bluebubbles", "robin-homeassistant", "robin-email", "robin-sms", "robin-mattermost", "robin-matrix", "robin-dingtalk", "robin-feishu", "robin-wecom", "robin-wecom-callback", "robin-weixin", "robin-qqbot", "robin-webhook", "robin-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (robin-<name>).
        # Gives them _ROBIN_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("robin-"):
            platform_name = name[len("robin-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ROBIN_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
