"""Centralized logging setup for Robin Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.robin/logs/`` (profile-aware via ``get_robin_home()``).
...
from robin_constants import get_config_path, get_robin_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_robin_session_injector", False):
    ...
    _session_record_factory._robin_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``robin logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_robin_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._robin_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the robin group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from robin_cli.config import is_managed
