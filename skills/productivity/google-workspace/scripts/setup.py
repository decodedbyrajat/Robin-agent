#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Robin Agent.
...
# Ensure sibling modules (_robin_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _robin_home import display_robin_home, get_robin_home

ROBIN_HOME = get_robin_home()
TOKEN_PATH = ROBIN_HOME / "google_token.json"
CLIENT_SECRET_PATH = ROBIN_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ROBIN_HOME / "google_oauth_pending.json"
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'robin-agent[google]'")
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_robin_home()}/google_token.json")
