...
ROBIN_HOME_DIR="${ROBIN_HOME:-$HOME/.robin}"
ROBIN_CFG="${ROBIN_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
if [[ ! -f "$ROBIN_CFG" ]]; then
    echo -e " ${FAIL} Robin config not found at ${ROBIN_CFG}"
    manual_steps+=("Create ${ROBIN_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ROBIN_CFG" 2>/dev/null; then
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ROBIN_CFG} manually"); }
