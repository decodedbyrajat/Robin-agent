...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ROBIN_HOME", os.path.expanduser("~/.robin")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ROBIN_HOME", Path.home() / ".robin")) / "skills" / "red-teaming" / "godmode"
...
ROBIN_HOME = Path(os.getenv("ROBIN_HOME", Path.home() / ".robin"))
CONFIG_PATH = ROBIN_HOME / "config.yaml"
PREFILL_PATH = ROBIN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Robin models — already uncensored, just needs clean prompt
    "robin": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "robin" in model_lower or "nous" in model_lower:
        return "robin"
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.robin/prefill.json."""
