...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Robin Agent\",
    \"comments\": [
...
```bash
source "${ROBIN_HOME:-$HOME/.robin}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Robin Agent. Code looks clean — good test coverage, no security concerns."
