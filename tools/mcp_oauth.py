    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Robin Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ROBIN_HOME so each profile gets its own OAuth tokens.
    Layout: ``ROBIN_HOME/mcp-tokens/``
    """
    try:
        from robin_constants import get_robin_home
        base = Path(get_robin_home())
    except ImportError:
        base = Path(os.environ.get("ROBIN_HOME", str(Path.home() / ".robin")))
...
class RobinTokenStorage:
    ...
    File layout::
        ...
        ROBIN_HOME/mcp-tokens/<server_name>.json         -- tokens
        ROBIN_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Robin Agent")
    scope = cfg.get("scope")
