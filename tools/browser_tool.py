...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    robin_node_bin = str(robin_home / "node" / "bin")
    return [robin_node_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-robin_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed robin processes — this way every clean robin exit sweeps
    ...
    # Sweep orphans from other crashed robin processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live robin processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current robin PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live robin process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning robin process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced robin PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent robin instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-robin_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live robin process.
            continue
...
def _run_browser_command(
    ...
    if _is_local_mode() and not _chromium_installed():
        if _running_in_docker():
            hint = (
                ...
                "docker pull ghcr.io/nousresearch/robin-agent:latest"
            )
        else:
            hint = (
    ...
    try:
        ...
        # Record this robin PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = robin_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    screenshots_dir = get_robin_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/robin-agent:latest")
                else:
