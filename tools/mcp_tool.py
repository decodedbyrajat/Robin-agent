...
discovers their tools, and registers them into the robin-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.robin/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.robin/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            robin_home = os.path.expanduser(
                os.getenv(
                    "ROBIN_HOME", os.path.join(os.path.expanduser("~"), ".robin")
                )
            )
            candidates = [
                os.path.join(robin_home, "node", "bin", resolved_command),
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `robin mcp login {server_name}` (or delete the tokens "
            f"file under ~/.robin/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.robin/.env`` loaded at startup).
    """
    try:
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``robin tools`` interactive configuration — connects to each
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple robin
