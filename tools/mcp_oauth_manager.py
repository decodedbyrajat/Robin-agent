...
`mcp_tool.py`, and `robin_cli/mcp_config.py`. This module is the ONLY place
...
def _make_robin_provider_class() -> Optional[type]:
    ...
    class RobinMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._robin_server_name = server_name
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._robin_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._robin_server_name, asm.token_endpoint,
                        )
                        break
...
_ROBIN_PROVIDER_CLS: Optional[type] = _make_robin_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _ROBIN_PROVIDER_CLS is None:
            logger.warning(
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``robin mcp remove <name>`` and (indirectly) by
        ``robin mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
