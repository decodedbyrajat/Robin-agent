...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.robin/config.yaml``. Robin
...
Configuration is loaded from ~/.robin/config.yaml under the 'tts:' key.
...
from tools.tool_backend_helpers import managed_iftv_tools_enabled, prefers_gateway, resolve_openai_audio_api_key
...
def _import_edge_tts():
    """Lazy import edge-tts. Returns the module or raises ImportError."""
    import edge_tts
    return edge_tts
...
def _get_default_output_dir() -> str:
    from robin_constants import get_robin_dir
    return str(get_robin_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.robin/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.robin/config.yaml.
    ...
    try:
        ...
        logger.debug("robin_cli.config not available, using default TTS config")
        return {}
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__ROBIN_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.robin/cache/piper-voices/`` under the active
    ROBIN_HOME so voice downloads follow profile boundaries.
    """
    from robin_constants import get_robin_dir
    root = Path(get_robin_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.robin/config.yaml (tts: section).
    ...
    platform = get_session_env("ROBIN_SESSION_PLATFORM", "").lower()
    ...
    try:
        ...
        if command_provider_config is not None:
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'robin-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run robin setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'robin setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'robin tools' and select Piper under TTS, or install manually: "
            ...
            # Default: OpenAI TTS (Kokoro/Local/Remote)
            logger.info("Generating speech with OpenAI TTS (default)...")
            _generate_openai_tts(text, file_str, tts_config)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_robin_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
