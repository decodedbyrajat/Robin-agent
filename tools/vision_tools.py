...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("ROBIN_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
            ...
            pass
    try:
        from robin_cli.config import cfg_get, load_config
        cfg = load_config()
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $ROBIN_HOME/cache/vision/ and cleaned up
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_robin_dir("cache/vision", "temp_vision_images")
