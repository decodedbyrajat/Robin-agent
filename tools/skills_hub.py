...
Used by robin_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            robin_meta = metadata.get("robin", {})
            if isinstance(robin_meta, dict):
                tags = robin_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by IfThenVoid) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.robin/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                robin_meta = meta_block.get("robin", {})
                if isinstance(robin_meta, dict):
                    tags = robin_meta.get("tags", [])
...
ROBIN_INDEX_URL = "https://github.com/IfThenVoid/Robin/tree/main/website/docs/api/skills-index.json"
ROBIN_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "robin-index.json"
ROBIN_INDEX_TTL = 6 * 3600  # 6 hours


def _load_robin_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ROBIN_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ROBIN_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ROBIN_INDEX_CACHE_FILE.stat().st_mtime
            if age < ROBIN_INDEX_TTL:
                return json.loads(ROBIN_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ROBIN_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
    ...
    # Cache locally
    try:
        ROBIN_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ROBIN_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class RobinIndexSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "robin-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "robin-index"),
            identifier=entry.get("identifier", ""),
