...
def managed_iftv_tools_enabled() -> bool:
    ...
    try:
        ...
        from robin_cli.models import check_nous_free_tier
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    managed_mode_blocked = (
        requested_mode == "managed" and not managed_iftv_tools_enabled()
    )

    if normalized_mode == "managed":
        selected_backend = "managed" if managed_iftv_tools_enabled() and managed_ready else None
    elif normalized_mode == "direct":
        selected_backend = "direct" if has_direct else None
    else:
        selected_backend = "managed" if managed_iftv_tools_enabled() and managed_ready else "direct" if has_direct else None
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.robin/.env`` (via
    ``robin_cli.config.get_env_value`` when available) so tool-side
