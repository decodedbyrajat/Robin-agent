...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ROBIN_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $ROBIN_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ROBIN_ENV_PATH = (
    r'(?:~\/\.robin/|'
    r'(?:\$home|\$\{home\})/\.robin/|'
    r'(?:\$robin_home|\$\{robin_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\brobin\s+gateway\s+(stop|restart)\b', "stop/restart robin gateway (kills running agents)"),
    (r'\brobin\s+update\b', "robin update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart robin-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart robin-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(robin|gateway|cli\.py)\b', "kill robin/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill robin` but not
    # `kill -9 $(pgrep -f robin)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("ROBIN_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("ROBIN_INTERACTIVE")
    is_gateway = os.getenv("ROBIN_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        # Cron sessions: respect cron_mode config
        if os.getenv("ROBIN_CRON_SESSION"):
    ...
    if is_gateway or os.getenv("ROBIN_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("ROBIN_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("ROBIN_INTERACTIVE")
    is_gateway = os.getenv("ROBIN_GATEWAY_SESSION")
    is_ask = os.getenv("ROBIN_EXEC_ASK")
