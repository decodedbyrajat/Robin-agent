...
from tools.environments.local import _ROBIN_PROVIDER_ENV_BLOCKLIST
...
def _load_robin_env_vars() -> dict[str, str]:
    """Load ~/.robin/.env values without failing Docker command execution."""
    try:
        from robin_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``ROBIN_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("ROBIN_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using ROBIN_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'robin'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.robin/sandboxes/). Non-persistent
        ...
        container_name = f"robin-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _ROBIN_PROVIDER_ENV_BLOCKLIST)
        robin_env = _load_robin_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = robin_env.get(key)
