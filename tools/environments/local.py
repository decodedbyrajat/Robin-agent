...
_ROBIN_PROVIDER_ENV_FORCE_PREFIX = "_ROBIN_FORCE_"
...
_ROBIN_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_ROBIN_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _ROBIN_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_ROBIN_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_ROBIN_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _ROBIN_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("ROBIN_GIT_BASH_PATH")
    ...
    raise RuntimeError(
        "Git Bash not found. Robin Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set ROBIN_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_ROBIN_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_ROBIN_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _ROBIN_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # npm …) into {ROBIN_HOME}/home/ when that directory exists.  Only the
...
class LocalEnvironment(BaseEnvironment):
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._robin_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_robin_pgid", None)
                    if pgid is None:
                        raise
