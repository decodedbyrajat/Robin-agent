...
def _is_robin_provider_credential(name: str) -> bool:
    """True if ``name`` is a Robin-managed provider credential (API key,
    token, or similar) per ``_ROBIN_PROVIDER_ENV_BLOCKLIST``.
    ...
    try:
        ...
        return False
    return name in _ROBIN_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    ``_ROBIN_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_robin_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Robin provider "
                "credential %r (blocked by _ROBIN_PROVIDER_ENV_BLOCKLIST). "
