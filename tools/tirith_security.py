...
it is automatically downloaded from GitHub releases to $ROBIN_HOME/bin/tirith.
...
def _get_robin_home() -> str:
    """Return the Robin home directory, respecting ROBIN_HOME env var."""
    return str(get_robin_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_robin_home(), ".tirith-install-failed")
...
def _robin_bin_dir() -> str:
    """Return $ROBIN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_robin_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ROBIN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_robin_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ROBIN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ROBIN_HOME/bin/tirith
    ...
    robin_bin = os.path.join(_robin_bin_dir(), "tirith")
    if os.path.isfile(robin_bin) and os.access(robin_bin, os.X_OK):
        _resolved_path = robin_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return robin_bin
