...
Architecture (two transports):
  ...
  **Local backend (UDS):**
  1. Parent generates a `robin_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `robin_tools.py` with file-based RPC stubs
...
def generate_robin_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the robin_tools.py stub module.
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["ROBIN_RPC_SOCKET"])
...
_RPC_DIR = os.environ.get("ROBIN_RPC_DIR") or os.path.join(tempfile.gettempdir(), "robin_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated robin_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/robin_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_robin_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/robin_tools.py", tools_src)
        ...
        env_prefix = (
            f"ROBIN_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ROBIN_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with robin_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="robin_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"robin_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated robin_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_robin_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "robin_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["ROBIN_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the robin-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from robin_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _robin_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _robin_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # ROBIN_TIMEZONE is an internal Robin setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ROBIN_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ROBIN_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {ROBIN_HOME}/home/ when that directory exists.
        from robin_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.robin/.env')).
...
def _load_config() -> dict:
    ...
    ``robin --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (robin-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``robin tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and robin-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.robin/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        ...
        f"Available via `from robin_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into robin_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from robin_tools import {import_str}` "
