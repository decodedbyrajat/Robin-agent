...
# 5. Symlinks the 'robin' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Robin Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink robin into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up robin command..."

ROBIN_BIN="$SCRIPT_DIR/venv/bin/robin"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ROBIN_BIN" "$COMMAND_LINK_DIR/robin"
echo -e "${GREEN}✓${NC} Symlinked robin → $COMMAND_LINK_DISPLAY_DIR/robin"
...
# ============================================================================
# Seed bundled skills into ~/.robin/skills/
# ============================================================================

ROBIN_SKILLS_DIR="${ROBIN_HOME:-$HOME/.robin}/skills"
mkdir -p "$ROBIN_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.robin/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ROBIN_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  robin status        # Check configuration"
if is_termux; then
    echo "  robin gateway       # Run gateway in foreground"
else
    echo "  robin gateway install # Install gateway service (messaging + cron)"
fi
echo "  robin cron list     # View scheduled jobs"
echo "  robin doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m robin_cli.main setup
fi
