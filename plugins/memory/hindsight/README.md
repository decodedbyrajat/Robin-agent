...
```bash
robin memory setup    # select "hindsight"
```
...
```bash
robin config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.robin/.env
```
...
Daemon startup logs: `~/.robin/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p robin ui start
```
...
Config file: `~/.robin/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `robin` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `robin-{profile}` isolates memory per active Robin profile. Empty placeholders collapse cleanly (e.g. `robin-{user}` with no user becomes `robin`). |
...
| `retain_context` | `conversation between Robin Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.robin/.env` as `HINDSIGHT_LLM_API_KEY`.
