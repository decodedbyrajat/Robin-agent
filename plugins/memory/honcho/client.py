...
Resolution order for config file:
  1. $ROBIN_HOME/honcho.json  (instance-local, enables isolated Robin instances)
...
def resolve_active_host() -> str:
    ...
    Resolution order:
      1. ROBIN_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``robin.<profile>``
      3. Fallback: ``"robin"`` (default profile)
    """
    explicit = os.environ.get("ROBIN_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ROBIN_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.robin/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_robin_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = Path.home() / ".robin" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.robin
    ...
    def from_global_config(
        ...
        Resolution: $ROBIN_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        ...
        # A hosts.robin block or explicit enabled flag means the user
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'robin honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            robin_cfg = load_config()
            honcho_cfg = robin_cfg.get("honcho", {})
