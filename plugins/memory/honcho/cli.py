...
Handles: robin honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "robin" else ""
...
def cmd_sync(args) -> None:
    ...
    """
    try:
        from robin_cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'robin honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'robin honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `robin update` -- no output, no exceptions.
    """
    try:
        from robin_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _local_config_path() -> Path:
    ...
    Always returns $ROBIN_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_robin_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    robin_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'robin honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = robin_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        robin_host["peerName"] = new_peer

    current_ai = robin_host.get("aiPeer") or cfg.get("aiPeer", "robin")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        robin_host["aiPeer"] = new_ai

    current_workspace = robin_host.get("workspace") or cfg.get("workspace", "robin")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        robin_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = robin_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        robin_host["observationMode"] = new_obs
    else:
        robin_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(robin_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        robin_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        robin_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = robin_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        robin_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = robin_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in ("none", "uncapped", "no limit"):
        robin_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                robin_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(robin_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            robin_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        robin_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        robin_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in ("minimal", "low", "medium", "high", "max"):
        robin_host["dialecticReasoningLevel"] = new_reasoning
    else:
        robin_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = robin_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        robin_host["sessionStrategy"] = new_strat

    robin_host["enabled"] = True
    robin_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        robin_config = load_config()
        robin_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(robin_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: robin config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    robin honcho status     -- show full config")
    print("    robin honcho mode       -- change recall/observation mode")
    print("    robin honcho tokens     -- tune context and dialectic budgets")
    print("    robin honcho peer       -- update peer names")
    print("    robin honcho map <name> -- map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: robin honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'robin honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: robin honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        robin = hosts.get(_host_key(), {})
        user = robin.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = robin.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = robin.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = robin.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Robin' identity in Honcho. Seed with 'robin honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: robin honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: robin honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = robin.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = robin.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = robin.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("\n  Set with: robin honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'robin honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'robin honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    robin honcho identity --show        — show both peer representations")
        print("    robin honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  robin honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'robin honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'robin honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  robin honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'robin'")
        ...
        for f in user_files:
            print("    robin honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'robin honcho setup' first, then re-run this step.")
    else:
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'robin honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    robin honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  robin honcho identity <path/to/SOUL.md>")
    ...
    print("              Map a custom name:  robin honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. robin honcho setup              — configure API key (required)")
        print("  2. robin honcho migrate            — re-run this walkthrough")
    else:
        print("  1. robin honcho status             — verify Honcho connection")
        print("  2. robin                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. robin honcho identity --show    — verify AI peer representation")
        print("  4. robin honcho tokens             — tune context and dialectic budgets")
        print("  5. robin honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'robin memory setup'...\n")
        from robin_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``robin honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``robin honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to robin memory setup)",
    )
