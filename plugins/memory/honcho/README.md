...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/robin>
...
```bash
robin honcho setup    # full interactive wizard (cloud or local)
robin memory setup    # generic picker, also works
```

Or manually:
```bash
robin config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.robin/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ROBIN_HOME/honcho.json` | Profile-local (isolated Robin instances) |
| 2 | `~/.robin/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Robin profile: `robin` (default) or `robin.<profile>`.
...
| 5 | `per-repo` strategy | Git root directory name (`robin-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`robin`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-robin-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening robin in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Robin session ID (timestamp + hex). Every `robin` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
...
Both profiles see the same user (`yourname`) in the same shared environment (`robin`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Robin profile: `robin` (default) or `robin.<profile>` (e.g. `robin -p coder` → host key `robin.coder`).
...
| `ROBIN_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `robin honcho setup` | Full interactive setup wizard |
| `robin honcho status` | Show resolved config for active profile |
| `robin honcho enable` / `disable` | Toggle Honcho for active profile |
| `robin honcho mode <mode>` | Change recall or observation mode |
| `robin honcho peer --user <name>` | Update user peer name |
| `robin honcho peer --ai <name>` | Update AI peer name |
| `robin honcho tokens --context <N>` | Set context token budget |
| `robin honcho tokens --dialectic <N>` | Set dialectic max chars |
| `robin honcho map <name>` | Map current directory to a session name |
| `robin honcho sync` | Create host blocks for all Robin profiles |
