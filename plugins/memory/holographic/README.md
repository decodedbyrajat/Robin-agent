...
```bash
robin memory setup    # select "holographic"
```

Or manually:
```bash
robin config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.robin-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ROBIN_HOME/memory_store.db` | SQLite database path |
