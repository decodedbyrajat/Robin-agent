"""disk_cleanup — ephemeral file cleanup for Robin Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ROBIN_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ROBIN_HOME and /tmp/robin-*
Never touches: ~/.robin/logs/ or any system directory.
"""
...
try:
    ...
    def get_robin_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ROBIN_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".robin").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ROBIN_HOME/logs/``."""
    return get_robin_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ROBIN_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ROBIN_HOME or ``/tmp/robin-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    robin_home = get_robin_home()
    try:
        path.resolve().relative_to(robin_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/robin-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("robin-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ROBIN_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ROBIN_HOME (but leave ROBIN_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "robin-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(robin_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == robin_home:
                continue
            try:
                rel_parts = dirpath.relative_to(robin_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    robin_home = get_robin_home()
    try:
        rel = path.resolve().relative_to(robin_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "robin-agent",
        }:
            return None
        ...
        # Path isn't under ROBIN_HOME (e.g. /tmp/robin-*) — fall through.
        pass
