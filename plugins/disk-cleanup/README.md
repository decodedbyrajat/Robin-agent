...
Auto-tracks and cleans up ephemeral files created during Robin Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ROBIN_HOME` and `/tmp/robin-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ROBIN_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ROBIN_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `ROBIN_HOME` or `/tmp/robin-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ROBIN_HOME/disk-cleanup/` is itself excluded
- `$ROBIN_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
