...
```bash
robin plugins enable google_meet
robin meet install                 # pip deps + Chromium (transcribe only)
robin meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
robin meet auth                    # optional; skips guest-lobby wait
robin meet setup                   # preflight checks
```

`robin meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or ROBIN_MEET_REALTIME_KEY in ~/.robin/.env
```
...
pip install playwright websockets && python -m playwright install chromium
robin plugins enable google_meet
robin meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
robin meet node approve my-mac ws://<mac-ip>:18789 <token>
robin meet node ping my-mac                   # confirm reachable
```

Run `robin meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `robin meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ROBIN_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `robin_meet_src`). |
...
Local:
```
$ROBIN_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ROBIN_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `robin meet node approve`.
