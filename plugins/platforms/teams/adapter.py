"""
Microsoft Teams platform adapter for Robin Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        robin_action = data.get("robin_action", "")
        session_key = data.get("session_key", "")

        if not robin_action or not session_key:
        ...
        choice = choice_map.get(robin_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="robin_approve",
                    data={**btn_data_base, "robin_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="robin_approve",
                    data={**btn_data_base, "robin_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="robin_approve",
                    data={**btn_data_base, "robin_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="robin_approve",
                    data={**btn_data_base, "robin_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print()
    print_success("Teams configuration saved to ~/.robin/.env")
    print_info("Install the app in Teams:  teams app install --id <teamsAppId>")
    print_info("Restart the gateway:       robin gateway restart")
