...
1. **Theme** — copy the theme YAML into your Robin home:

   ```
   cp theme/strike-freedom.yaml ~/.robin/dashboard-themes/
   ```
...
2. **Plugin** — the `dashboard/` directory gets auto-discovered because
   ...
   whole plugin directory into `~/.robin/plugins/`:

   ```
   cp -r . ~/.robin/plugins/strike-freedom-cockpit
   ```
