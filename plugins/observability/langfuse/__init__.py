...
load when listed in ``plugins.enabled`` (via ``robin plugins enable
observability/langfuse`` or ``robin tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``robin tools`` or ~/.robin/.env):
  ROBIN_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  ROBIN_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  ROBIN_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  ROBIN_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  ROBIN_LANGFUSE_RELEASE     - release/version tag
  ROBIN_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  ROBIN_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  ROBIN_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("ROBIN_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    public_key = _env("ROBIN_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("ROBIN_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    if not (public_key and secret_key):
        _LANGFUSE_CLIENT = _INIT_FAILED
        return None

    base_url = _env("ROBIN_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("ROBIN_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("ROBIN_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("ROBIN_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid ROBIN_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("ROBIN_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
