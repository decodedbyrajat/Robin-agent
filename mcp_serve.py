...
logger = logging.getLogger("robin.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ROBIN_HOME."""
    try:
        from robin_constants import get_robin_home
        return get_robin_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ROBIN_HOME", Path.home() / ".robin")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from robin_constants import get_robin_home
        directory_file = get_robin_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ROBIN_HOME", Path.home() / ".robin")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            from robin_constants import get_robin_home
            db_file = get_robin_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ROBIN_HOME", Path.home() / ".robin")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    ...
    mcp = FastMCP(
        "robin",
        instructions=(
            "Robin Agent messaging bridge. Use these tools to interact with "
