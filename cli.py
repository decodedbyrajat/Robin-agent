#!/usr/bin/env python3
"""
Robin Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Robin Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills robin-agent-dev,github-auth
...
os.environ["ROBIN_QUIET"] = "1"  # Our own modules
...
from robin_cli.banner import _format_context_length, format_banner_version_label
...
from robin_cli.browser_connect import (
...
load_robin_dotenv(robin_home=_robin_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.robin/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If ROBIN_IGNORE_USER_CONFIG=1 is set (via ``robin chat --ignore-user-config``),
    the user config at ``~/.robin/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({ROBIN_HOME}/config.yaml)
    user_config_path = _robin_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("ROBIN_IGNORE_USER_CONFIG") == "1"
    ...
    if config_path.exists():
        try:
            ...
            # location is model.provider (written by `robin model`).
    ...
    # Normalize config key: the new config system (robin_cli/config.py) and all
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["ROBIN_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.robin/logs/.
...
try:
    from robin_cli.skin_engine import init_skin_from_config
...
from robin_cli.banner import build_welcome_banner
from robin_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from robin_cli.callbacks import prompt_for_secret
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run robin -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"robin-{short_id}"
    branch_name = f"robin/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`robin_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from robin_constants import get_robin_home as _get_robin_home
        ...
        session_db.maybe_auto_prune_and_vacuum(
            ...
            sessions_dir=_get_robin_home() / "sessions",
        )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`robin_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``robin/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("robin-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``robin/robin-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``robin -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("robin/robin-") or b.startswith("pr-"))
    ]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from robin_cli.skin_engine import get_active_skin
...
from robin_constants import is_termux as _is_termux_environment
...
# ASCII Art - ROBIN-AGENT logo (full width, single line - requires ~95 char terminal)
ROBIN_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "Robin - AI Agent by IfThenVoid"
        tiny_line = "Robin"
    else:
        agent_name = _skin.get_branding("agent_name", "Robin Agent") if _skin else "Robin Agent"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- IfThenVoid[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from robin_cli.plugins import get_plugin_manager
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.robin/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.robin/config.yaml on first use)
...
class RobinCLI:
    """
    Interactive CLI for the Robin Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from robin_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("ROBIN_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("ROBIN_MAX_ITERATIONS"):
            self.max_turns = int(os.getenv("ROBIN_MAX_ITERATIONS"))
        else:
            self.max_turns = 90
        ...
        # by `robin chat --ignore-rules` in robin_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("ROBIN_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("ROBIN_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Robin processes for this ROBIN_HOME.
        ...
        self._history_file = _robin_home / ".robin_history"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from robin_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from robin_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from robin_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: robin setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: robin setup")
            return False
        ...
        # `robin chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `robin auth add openai-codex`
        # without `robin model`), fall back to the provider's first catalog
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (robin sessions list).{_RST}")
                return False
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   Robin needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        # Warn if the configured model is a non-agentic Robin LLM.
        from robin_cli.model_switch import is_nous_robin_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_robin_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Robin 3 & 4 models are NOT agentic and are not "
                "designed for use with Robin Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.robin/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from robin_cli.clipboard import save_clipboard_image

        img_dir = get_robin_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: robin --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        if subcmd in ("list", "ls"):
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_robin_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from robin_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run robin chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'robin setup' to configure[/]")
        except Exception:
    ...
    def show_help(self):
        ...
        from robin_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from robin_cli.skin_engine import get_active_help_header
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in robin_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from robin_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("ROBIN_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `robin sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from robin_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `robin sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.robin/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``robin --resume <id>``
        ...
        saved_dir = get_robin_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"robin_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: robin --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from robin_cli.curses_ui import curses_single_select
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `robin model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from robin_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from robin_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from robin_cli.model_switch import switch_model, parse_model_flags, list_authenticated_providers
        ...
        custom_provs = None
        try:
            from robin_cli.config import get_compatible_custom_providers, load_config
            cfg = load_config()
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from robin_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to robin_cli.curator so the CLI and the `robin curator`
        ...
        try:
            from robin_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to robin_cli.skills_hub."""
        from robin_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_robin_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_robin_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in robin_cli/commands.py instead of touching every dispatch site.
        from robin_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in ("quit", "exit", "q"):
            ...
            from robin_cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_robin_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from robin_cli.commands import COMMANDS
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from robin_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_robin_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from robin_cli.config import load_config
        from robin_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = bool(os.environ.get("ROBIN_YOLO_MODE"))
        if current:
            os.environ.pop("ROBIN_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from robin_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from robin_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            for quiet_logger in ('tools', 'run_agent', 'trajectory_compressor', 'cron', 'robin_cli'):
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from robin_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.robin/skills/ and queue a note for the
        next user turn.
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "robin_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _robin_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  robin --resume {self.session_id}")
            if session_title:
                print(f"  robin -c \"{session_title}\"")
            print()
            ...
            try:
                from robin_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from robin_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from robin_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        # Only show when the user explicitly configured Honcho for Robin
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Robin Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Robin Agent! Type your message or /help for commands."
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from robin_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from robin_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Robin Agent")
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``ROBIN_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("ROBIN_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: robin setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    Robin Agent CLI - Interactive AI Assistant
    ...
    os.environ["ROBIN_INTERACTIVE"] = "1"
    ...
    # Default to robin-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # so main unwinds normally.  ROBIN_SIGTERM_GRACE overrides the 1.5 s
