...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `ROBIN_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("ROBIN_TUI_SIDECAR_URL")
...
# tested; override via ``ROBIN_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("ROBIN_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``ROBIN_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
