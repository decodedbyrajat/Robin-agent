...
as ``kind: backend``) or ``~/.robin/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``robin tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``robin tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``robin tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$ROBIN_HOME/cache/images/``, creating parents as needed."""
    from robin_constants import get_robin_home

    path = get_robin_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ROBIN_HOME/cache/images/``.
