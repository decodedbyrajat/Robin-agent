...
def _state_file() -> Path:
    return get_robin_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.robin/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    ...
    "into ~/.robin/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: robin-config-*, robin-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.robin/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.robin/logs/curator/``)
    ...
    authored skill data in ``~/.robin/skills/``.
    """
    return get_robin_home() / "logs" / "curator"
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if archived:
        ...
        lines.append("_Archived skills are at `~/.robin/skills/.archive/`. "
                     "Restore any via `robin curator restore <name>`._\n")
    ...
    lines.append("- Restore an archived skill: `robin curator restore <name>`")
    lines.append("- All archives live under `~/.robin/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    def _llm_pass():
        ...
        # recorded in state so `robin curator status` can point at it.
        try:
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``robin model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `robin model` → auxiliary
