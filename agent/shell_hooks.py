...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`robin_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.robin/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``ROBIN_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``robin_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``robin_cli.config.load_config``
    ...
    setting.  ``ROBIN_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / ROBIN_ACCEPT_HOOKS=1 / "
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``robin hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from robin_cli.plugins import VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`robin_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def allowlist_path() -> Path:
    ...
    return get_robin_home() / ALLOWLIST_FILENAME
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / ROBIN_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``ROBIN_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("ROBIN_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `robin hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``robin hooks test`` and ``robin hooks doctor``.

    ``kwargs`` is the same dict that :func:`robin_cli.plugins.invoke_hook`
    ...
    produce — otherwise scripts tested via ``robin hooks test`` could
