...
# `export ROBIN_REDACT_SECRETS=true`) cannot enable/disable redaction
# mid-session.  OFF by default — user must opt in via
# `security.redact_secrets: true` in config.yaml (bridged to this env var
# in robin_cli/main.py and gateway/run.py) or `ROBIN_REDACT_SECRETS=true`
# in ~/.robin/.env.
_REDACT_ENABLED = os.getenv("ROBIN_REDACT_SECRETS", "").lower() in ("1", "true", "yes", "on")
...
def mask_secret(
    ...
    *,
    ...
    ``robin config``, ``robin status``, ``robin dump``, and anywhere
