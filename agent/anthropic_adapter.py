"""Anthropic Messages API adapter for Robin Agent.
...
# Stores credentials in ~/.robin/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_ROBIN_OAUTH_FILE = get_robin_home() / ".anthropic_oauth.json"
...
def run_robin_oauth_login_pure() -> Optional[Dict[str, Any]]:
...
def read_robin_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Robin-managed OAuth credentials from ~/.robin/.anthropic_oauth.json."""
    if _ROBIN_OAUTH_FILE.exists():
        try:
            data = json.loads(_ROBIN_OAUTH_FILE.read_text(encoding="utf-8"))
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See robin-agent#13848.
            #
    ...
    # are preserved.  See robin-agent#13848 (Kimi) and #16748 (DeepSeek).
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                ...
                text = text.replace("Robin agent", "Claude Code")
                text = text.replace("robin-agent", "claude-code")
                text = text.replace("IfThenVoid", "Anthropic")
                block["text"] = text
