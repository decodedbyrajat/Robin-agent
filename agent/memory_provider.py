...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - robin_home (str): The active ROBIN_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.robin``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "robin").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'robin memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], robin_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'robin memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``robin_home`` is the active ROBIN_HOME directory path.
