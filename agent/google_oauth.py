...
Storage (``~/.robin/auth/google_oauth.json``, chmod 0o600):
...
#
# Resolution order:
#   1. ROBIN_GEMINI_CLIENT_ID / ROBIN_GEMINI_CLIENT_SECRET env vars (power users)
...
ENV_CLIENT_ID = "ROBIN_GEMINI_CLIENT_ID"
ENV_CLIENT_SECRET = "ROBIN_GEMINI_CLIENT_SECRET"
...
_HEADLESS_ENV_VARS = ("SSH_CONNECTION", "SSH_CLIENT", "SSH_TTY", "ROBIN_HEADLESS")
...
def _credentials_path() -> Path:
    return get_robin_home() / "auth" / "google_oauth.json"
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "  2. Set ROBIN_GEMINI_CLIENT_ID and ROBIN_GEMINI_CLIENT_SECRET in ~/.robin/.env\n"
            "\n"
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `robin login --provider google-gemini-cli` first.",
...
def start_oauth_flow(
    *,
    ...
    auth_url = AUTH_ENDPOINT + "?" + urllib.parse.urlencode(params) + "#robin"
