...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Robin also honours ROBIN_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("ROBIN_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run Robin Agent.  Models with fewer
...
def _get_context_cache_path() -> Path:
    ...
    return get_robin_home() / "context_length_cache.yaml"
...
def get_model_context_length(
    ...
    if effective_provider in ("copilot", "copilot-acp", "github-copilot"):
        try:
            from robin_cli.models import get_copilot_model_context
