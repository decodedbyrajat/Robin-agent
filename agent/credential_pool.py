...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``robin model`` / ``robin auth`` which
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("robin_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("robin_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # re-authed via `robin model` / `robin auth` after a 429/401,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `robin auth remove <provider> <N>` is stable across all source types.
    try:
        from robin_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        try:
            from robin_cli.auth import is_provider_explicitly_configured
            if not is_provider_explicitly_configured("anthropic"):
                return changed, active_sources
        except ImportError:
            pass

        from agent.anthropic_adapter import read_claude_code_credentials, read_robin_oauth_credentials

        for source_name, creds in (
            ("robin_pkce", read_robin_oauth_credentials()),
        ...
        if state and not _is_suppressed(provider, "device_code"):
            ...
            # `robin auth add nous --label <name>`).  Fall back to the
        ...
        try:
            from robin_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.robin/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `robin auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `robin auth remove openai-codex` marks
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    changed = False
    active_sources: Set[str] = set()
    # Honour user suppression — `robin auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.robin/.env.
    ...
    try:
        from robin_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "robin_pkce"}
        )
    ]
