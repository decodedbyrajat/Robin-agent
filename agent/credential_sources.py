...
Robin seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.robin/.env
    claude_code   — ~/.claude/.credentials.json
    robin_pkce   — ~/.robin/.anthropic_oauth.json
    ...
    manual        — user ran `robin auth add`
...
unify here is **removal**:
    ...
    ``robin auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), robin_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.robin/.env  → clear it
    ...
    """
    from robin_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"{env_var} until you run `robin auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `robin auth add anthropic` to re-enable if needed.",
    ])


def _remove_robin_pkce(provider: str, removed) -> RemovalResult:
    """~/.robin/.anthropic_oauth.json is ours — delete it outright."""
    from robin_constants import get_robin_home

    result = RemovalResult()
    oauth_file = get_robin_home() / ".anthropic_oauth.json"
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `robin login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `robin auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    via ``robin auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from robin_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `robin auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `robin auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `robin auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="robin_pkce",
        remove_fn=_remove_robin_pkce,
        description="~/.robin/.anthropic_oauth.json",
    ))
    register(RemovalStep(
