...
Jobs are stored in ~/.robin/cron/jobs.json
Output is saved to ~/.robin/cron/output/{job_id}/{timestamp}.md
"""
...
ROBIN_DIR = get_robin_home().resolve()
CRON_DIR = ROBIN_DIR / "cron"
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    try:
        ...
        run_at = _robin_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall robin-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def save_jobs(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _robin_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _robin_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _robin_now().isoformat(),
        },
    )
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _robin_now().strftime("%Y-%m-%d_%H-%M-%S")
