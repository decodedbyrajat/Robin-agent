...
Interpreter resolution order is: `ROBIN_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.robin/.robin_history` or under `ROBIN_HOME`.
...
```text
ui-tui/
  packages/robin-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      robin-ink.d.ts    type declarations for @robin/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent RobinCLI subprocess for slash commands
```
