import type { RunExternalProcess } from '@robin/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`robin ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching robin: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`robin ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
