...
// Mirrors @robin/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @robin/ink is bundled separately from packages/.
...
// Terminal. Explicit ROBIN_TUI_THEME / COLORFGBG signals above still win,
...
// `ROBIN_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `ROBIN_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `ROBIN_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `ROBIN_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
export function detectLightMode(
  ...
  const lightFlag = (env.ROBIN_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.ROBIN_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.ROBIN_TUI_BACKGROUND ?? '')
