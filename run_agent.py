...
from robin_cli.timeouts import (
...
_loaded_env_paths = load_robin_dotenv(robin_home=_robin_home, project_env=_project_env)
...
from agent.prompt_builder import (
    ...
    ROBIN_AGENT_HELP_GUIDANCE,
    build_iftv_subscription_prompt,
)
...
from agent.routing.dispatch_hook import maybe_route_local
...
class _SafeWriter:
    ...
    When robin-agent runs as a systemd service, Docker container, or headless
...
def _routermint_headers() -> dict:
    ...
    from robin_cli import __version__ as _ROBIN_VERSION

    return {
        "User-Agent": f"RobinAgent/{_ROBIN_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[robin-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        Args:
            ...
            load_soul_identity (bool): If True, still use ~/.robin/SOUL.md as the primary
        ...
        self._cache_ttl = "5m"
        try:
            from robin_cli.config import load_config as _load_pc_cfg
        ...
        # both live under ~/.robin/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from robin_logging import setup_logging, setup_verbose_logging
        setup_logging(robin_home=_robin_home)
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from robin_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://github.com/IfThenVoid/Robin",
                        "X-OpenRouter-Title": "Robin Agent",
                        "X-OpenRouter-Categories": "productivity,cli-agent",
                    }
                elif base_url_host_matches(effective_base, "api.routermint.com"):
                    client_kwargs["default_headers"] = _routermint_headers()
                elif base_url_host_matches(effective_base, "api.githubcopilot.com"):
                    from robin_cli.models import copilot_default_headers
                ...
                if _routed_client is not None:
                    ...
                    # Preserve any default_headers / default_query the router set
                    # Check both the client itself and its inner _real_client (CodexAuxiliaryClient)
                    _src = getattr(_routed_client, "_real_client", _routed_client)
                    _headers = getattr(_src, "_custom_headers", None) or getattr(_src, "default_headers", None)
                    if _headers:
                        client_kwargs["default_headers"] = dict(_headers)
                    _query = getattr(_src, "_custom_query", None) or getattr(_src, "default_query", None)
                    if _query:
                        client_kwargs["default_query"] = dict(_query)
                else:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `robin model`."
                        )
                    # No provider configured — reject with a clear message.
                    raise RuntimeError(
                        "No LLM provider configured. Run `robin model` to "
                        "select a provider, or run `robin setup` for first-time "
                        "configuration."
                    )
            ...
            # Preserve full URL for downstream auxiliary clients if we stripped queries into default_query
            if "default_query" not in client_kwargs:
        ...
        # Session logs go into ~/.robin/sessions/ alongside gateway sessions
        robin_home = get_robin_home()
        self.logs_dir = robin_home / "sessions"
        ...
        if self._session_db:
            try:
                self._session_db.create_session(
                    ...
                    source=self.platform or os.environ.get("ROBIN_SESSION_SOURCE", "cli"),
                    model=self.model,
                    model_config={
        ...
        try:
            from robin_cli.config import load_config as _load_agent_config
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "robin"
                        except Exception:
                            pass
        ...
        if _config_context_length is None and _custom_providers:
            try:
                from robin_cli.config import get_custom_provider_context_length
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from robin_cli.plugins import get_plugin_context_engine
        ...
        # RAG memory: how many recent messages to keep in the active context window
        # (0 = disabled, send full history as usual)
        self._rag_max_recent = cfg_get(_agent_cfg, "rag_memory", "max_recent_messages", default=0)
        if self._rag_max_recent:
            logger.info("RAG memory: limiting active context to last %d messages", self._rag_max_recent)
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Robin Agent.  Choose a model with at least "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        ...
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from robin_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        turn-scoped).
        """
        from robin_cli.providers import determine_api_mode
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            ...
            try:
                from robin_cli.config import load_config, get_compatible_custom_providers
                _sm_cfg = load_config()
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``robin chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `robin setup` or set OPENROUTER_API_KEY."
                )
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ROBIN_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ROBIN_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("ROBIN_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ROBIN_API_CALL_STALE_TIMEOUT`` env var
          4. 300.0s default
        ...
        env_timeout = os.getenv("ROBIN_API_CALL_STALE_TIMEOUT")
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from robin_cli.models import _should_use_copilot_responses_api
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        metadata: Dict[str, Any] = {
            ...
            "platform": self.platform or os.environ.get("ROBIN_SESSION_SOURCE", "cli"),
            "tool_name": "memory",
        }
        if task_id:
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("ROBIN_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Some execution modes (cron) still want ROBIN_HOME persona while keeping
        ...
        prompt_parts.append(ROBIN_AGENT_HELP_GUIDANCE)
        ...
        iftv_subscription_prompt = build_iftv_subscription_prompt(self.valid_tool_names)
        if iftv_subscription_prompt:
            prompt_parts.append(iftv_subscription_prompt)
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the robin-agent install
        ...
        from robin_time import now as _robin_now
        now = _robin_now()
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        # Extract query params (e.g. Azure api-version) from base_url if not already present
        # as an explicit default_query. This prevents loss during SDK URL joining.
        _base = client_kwargs.get("base_url")
        if _base and "?" in str(_base) and "default_query" not in client_kwargs:
            from agent.auxiliary_client import _extract_url_query_params
            _clean_base, _dq = _extract_url_query_params(str(_base))
            if _dq:
                client_kwargs["base_url"] = _clean_base
                client_kwargs["default_query"] = _dq
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from robin_cli.copilot_auth import copilot_request_headers
    ...
    def _build_routed_openai_client(self, base_url: str, api_key: str):
        """Build an ephemeral OpenAI client for an intent-routed endpoint."""
        kwargs = dict(self._client_kwargs)
        kwargs["base_url"] = base_url
        kwargs["api_key"] = api_key
        return self._create_openai_client(kwargs, reason="intent_routed", shared=False)
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from robin_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("ROBIN_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("ROBIN_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from robin_cli.copilot_auth import resolve_copilot_token
        ...
        def _call():
            try:
                if self.api_mode == "codex_responses":
                    ...
                    routed = maybe_route_local(
                        api_kwargs,
                        self.api_mode,
                        build_local_client=self._build_routed_openai_client,
                    )
                    if routed is not None:
                        new_kwargs, routed_client, _ = routed
                        try:
                            result["response"] = routed_client.chat.completions.create(**new_kwargs)
                        finally:
                            self._close_request_openai_client(routed_client, reason="intent_routed_complete")
                    else:
                        request_client_holder["client"] = self._create_request_openai_client(
    ...
    def _interruptible_streaming_api_call(
        ...
        # If the intent router routes this to a local model, it forces stream=False.
        # Make the call directly using the routed client — do NOT re-enter
        # _interruptible_api_call(new_kwargs) because that would call
        # maybe_route_local again inside the thread.  The second call would see
        # model=mistral-small (already local), skip routing, then try to build
        # an Azure client for a local model name — wrong endpoint, wrong auth.
        if self.api_mode == "chat_completions":
            ...
            if routed is not None:
                new_kwargs, routed_client, _ = routed
                if new_kwargs.get("stream") is False:
                    try:
                        return routed_client.chat.completions.create(**new_kwargs)
                    finally:
                        self._close_request_openai_client(
                            routed_client, reason="intent_routed_complete"
                        )
        ...
        def _call_chat_completions():
            ...
            # wins over the ROBIN_API_TIMEOUT env default if the user set it.
            _provider_timeout_cfg = get_provider_request_timeout(self.provider, self.model)
            _base_timeout = (
                _provider_timeout_cfg
                if _provider_timeout_cfg is not None
                else float(os.getenv("ROBIN_API_TIMEOUT", 1800.0))
            )
            # Read timeout: config wins here too.  Otherwise use
            # ROBIN_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
            if _provider_timeout_cfg is not None:
                _stream_read_timeout = _provider_timeout_cfg
            else:
                _stream_read_timeout = float(os.getenv("ROBIN_STREAM_READ_TIMEOUT", 120.0))
                # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
                # prefill on large contexts before producing the first token.
                # Auto-increase the httpx read timeout unless the user explicitly
                # overrode ROBIN_STREAM_READ_TIMEOUT.
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("ROBIN_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("ROBIN_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set ROBIN_STREAM_STALE_TIMEOUT.
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        try:
            ...
            try:
                from robin_cli.model_normalize import normalize_model_for_provider
        ...
        def _shrink_data_url(url: str) -> Optional[str]:
            ...
            try:
                ...
                tmp = tempfile.NamedTemporaryFile(
                    prefix="robin_shrink_", suffix=suffix, delete=False,
                )
                try:
                    tmp.write(raw)
                    tmp.close()
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if _is_qwen:
            _qwen_meta = {
                "sessionId": self.session_id or "robin",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from robin_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from robin_cli.models import lmstudio_model_reasoning_options
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None) -> tuple:
        ...
        # compression (not a fresh /new). Plugin engines (e.g. robin-lcm) use
        # boundary_reason="compression" to preserve DAG lineage across the
        # rollover instead of re-initializing fresh per-session state.
        # See robin-lcm#68. Built-in ContextCompressor ignores kwargs.
        try:
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        try:
            from robin_cli.plugins import get_pre_tool_call_block_message
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=robin-agent"]
    ...
    def run_conversation(
        ...
        # ``robin logs --session <id>`` can filter a single conversation.
        from robin_logging import set_session_context
        ...
        # Add user message.
        #
        # Dedupe guard: when a previous attempt at this same turn failed after
        # persisting the session (non-retryable API error), the history already
        # ends with this exact user message and no assistant reply. Appending
        # again would send the message twice (and grow by one copy per failed
        # attempt). Reuse the dangling tail instead. Genuine repeats ("yes" ...
        # "yes") always have an assistant reply in between, so they never hit
        # this branch.
        user_msg = {"role": "user", "content": user_message}
        # Collapse the whole trailing run so sessions damaged by earlier
        # failed attempts ([hey, hey] already persisted) heal here too.
        while (
            messages
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # RAG memory: truncate to recent messages to reduce token usage.
            # This drops older conversation turns; relevant past context is
            # recovered via the memory provider's prefetch() instead.
            if self._rag_max_recent and len(messages) > self._rag_max_recent:
                messages = messages[-self._rag_max_recent:]
                current_turn_user_idx = len(messages) - 1
            ...
            while retry_count < max_retries:
                ...
                if self.provider == "nous":
                    try:
                        from agent.iftv_rate_guard import (
                            iftv_rate_limit_remaining,
                            format_remaining as _fmt_nous_remaining,
                        )
                        _nous_remaining = iftv_rate_limit_remaining()
                ...
                try:
                    ...
                    if env_var_enabled("ROBIN_DUMP_REQUESTS"):
                    ...
                    if self.provider == "nous":
                        try:
                            from agent.iftv_rate_guard import clear_iftv_rate_limit
                            clear_iftv_rate_limit()
                        except Exception:
                            pass
                    ...
                    if (
                        ...
                        print(f"{self.log_prefix}     • Re-authenticate: robin login --provider nous")
                    ...
                    if (
                        ...
                        from robin_constants import display_robin_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Robin-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: robin config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: robin config set ANTHROPIC_API_KEY \"\"")
                    ...
                    # ``is_genuine_iftv_rate_limit`` to tell the two
                    ...
                    if (
                        ...
                        _genuine_iftv_rate_limit = False
                        try:
                            from agent.iftv_rate_guard import (
                                is_genuine_iftv_rate_limit,
                                record_iftv_rate_limit,
                            )
                            ...
                            _genuine_iftv_rate_limit = is_genuine_iftv_rate_limit(
                                headers=_err_hdrs,
                                last_known_state=self._rate_limit_state,
                            )
                            if _genuine_iftv_rate_limit:
                                record_iftv_rate_limit(
                                    headers=_err_hdrs,
                                    error_context=error_context,
                                )
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `robin auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: robin setup", force=True)
