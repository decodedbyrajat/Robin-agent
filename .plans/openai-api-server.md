# OpenAI-Compatible API Server for Robin Agent
...
SSE streaming. By exposing this endpoint, robin-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  robin-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.robin/.env`
2. Run `robin gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with robin-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns robin-agent as a model) |
...
Authorization: Bearer robin-api-key-here
...
But robin-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "robin-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured robin-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of robin-agent's core prompt
...
Once implemented, robin-agent works as a drop-in backend for:
