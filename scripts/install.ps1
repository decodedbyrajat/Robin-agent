# ============================================================================
# Robin Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/IfThenVoid/Robin/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$RobinHome = "$env:LOCALAPPDATA\robin",
    [string]$InstallDir = "$env:LOCALAPPDATA\robin\robin-agent"
)
...
$RepoUrlSsh = "git@github.com:IfThenVoid/Robin.git"
$RepoUrlHttps = "https://github.com/IfThenVoid/Robin.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Robin Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
    Write-Host "│  Robin made by IfThenVoid.                             │" -ForegroundColor Magenta
    Write-Host "└─────────────────────────────────────────────────────────┘" -ForegroundColor Magenta
    Write-Host ""
}
...
function Test-Node {
    ...
    # Fallback: download binary zip to ~/.robin/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\robin-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$RobinHome\node") { Remove-Item -Recurse -Force "$RobinHome\node" }
                Move-Item $extractedDir.FullName "$RobinHome\node"
                $env:Path = "$RobinHome\node;$env:Path"

                $version = & "$RobinHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.robin/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/IfThenVoid/Robin/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\robin-agent-$Branch.zip"
                $extractPath = "$env:TEMP\robin-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up robin command..."
    
    if ($NoVenv) {
        $robinBin = "$InstallDir"
    } else {
        $robinBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so robin is globally available
    # On Windows, the robin.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$robinBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$robinBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $robinBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ROBIN_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\robin instead
    # of the Unix default ~/.robin
    $currentRobinHome = [Environment]::GetEnvironmentVariable("ROBIN_HOME", "User")
    if (-not $currentRobinHome -or $currentRobinHome -ne $RobinHome) {
        [Environment]::SetEnvironmentVariable("ROBIN_HOME", $RobinHome, "User")
        Write-Success "Set ROBIN_HOME=$RobinHome"
    }
    $env:ROBIN_HOME = $RobinHome
    
    # Update current session
    $env:Path = "$robinBin;$env:Path"
    
    Write-Success "robin command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.robin directory structure
    New-Item -ItemType Directory -Force -Path "$RobinHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RobinHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RobinHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RobinHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RobinHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RobinHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RobinHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RobinHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$RobinHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$RobinHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.robin/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.robin/.env"
        }
    } else {
        Write-Info "~/.robin/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$RobinHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.robin/config.yaml from template"
        }
    } else {
        Write-Info "~/.robin/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$RobinHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.robin/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.robin/"
    
    # Seed bundled skills into ~/.robin/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.robin/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.robin/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$RobinHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.robin/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        try {
            ...
            Write-Warn "TUI npm install failed (robin --tui may not work)"
        }
        Pop-Location
    }
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m robin_cli.main setup
    } else {
        python -m robin_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $robinCmd = "$InstallDir\venv\Scripts\robin.exe"
    if (-not (Test-Path $robinCmd)) {
        $robinCmd = "robin"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$RobinHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'robin whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$RobinHome\logs\gateway.log"
            Start-Process -FilePath $robinCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$RobinHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: robin gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: robin gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$RobinHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$RobinHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$RobinHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$RobinHome\robin-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   robin              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   robin setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   robin config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   robin config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   robin gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   robin update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/IfThenVoid/Robin/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
