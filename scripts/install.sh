...
# Configuration
REPO_URL_SSH="git@github.com:IfThenVoid/Robin.git"
REPO_URL_HTTPS="https://github.com/IfThenVoid/Robin.git"
ROBIN_HOME="${ROBIN_HOME:-$HOME/.robin}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ROBIN_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ROBIN_INSTALL_DIR"
...
#   code at /usr/local/lib/robin-agent, command at /usr/local/bin/robin,
#   data still at /root/.robin (ROBIN_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.robin/robin-agent"
            echo "                   default (root, Linux): /usr/local/lib/robin-agent"
            echo "  --robin-home PATH  Data directory (default: ~/.robin, or \$ROBIN_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Robin installs the code under"
            echo "  /usr/local/lib/robin-agent and links the command into"
            echo "  /usr/local/bin/robin (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ROBIN_HOME"
            echo "  (default /root/.robin).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ROBIN_HOME/robin-agent are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `robin` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ROBIN_HOME/robin-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ROBIN_HOME/robin-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/robin-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ROBIN_HOME/robin-agent — then preserve it)
#
# Always no-op when the user set --dir or $ROBIN_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ROBIN_HOME.
    if is_termux; then
        INSTALL_DIR="$ROBIN_HOME/robin-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ROBIN_HOME/robin-agent/.git" ]; then
            INSTALL_DIR="$ROBIN_HOME/robin-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/robin-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/robin-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/robin"
        log_info "  Data:    $ROBIN_HOME (unchanged)"
        return 0
    fi
...
get_robin_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/robin" ]; then
        echo "$link_dir/robin"
    else
        echo "robin"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/IfThenVoid/Robin/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$ROBIN_HOME/node/bin/node" ]; then
        export PATH="$ROBIN_HOME/node/bin:$PATH"
        local found_ver=$("$ROBIN_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Robin-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.robin/node/..."
    ...
    # Place into ~/.robin/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$ROBIN_HOME/node"
    mkdir -p "$ROBIN_HOME"
    mv "$extracted_dir" "$ROBIN_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$ROBIN_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$ROBIN_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$ROBIN_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$ROBIN_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$ROBIN_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.robin/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Robin Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="robin-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
...
setup_path() {
    log_info "Setting up robin command..."

    if [ "$USE_VENV" = true ]; then
        ROBIN_BIN="$INSTALL_DIR/venv/bin/robin"
    else
        ROBIN_BIN="$(which robin 2>/dev/null || echo "")"
        if [ -z "$ROBIN_BIN" ]; then
            log_warn "robin not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ROBIN_BIN" ]; then
        log_warn "robin entry point not found at $ROBIN_BIN"
    ...
    # Create a user-facing shim for the robin command.
    mkdir -p "$command_link_dir"
    ln -sf "$ROBIN_BIN" "$command_link_dir/robin"
    log_success "Symlinked robin → $command_link_display_dir/robin"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "robin command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v robin' \
                >/dev/null 2>&1; then
        ...
        log_info "robin not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Robin Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Robin Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Robin Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so robin works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.robin directory structure (config at top level, code in subdir)
    mkdir -p "$ROBIN_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.robin/.env (top level, easy to find)
    if [ ! -f "$ROBIN_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ROBIN_HOME/.env"
            log_success "Created ~/.robin/.env from template"
        else
            touch "$ROBIN_HOME/.env"
            log_success "Created ~/.robin/.env"
        fi
    else
        log_info "~/.robin/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.robin/config.yaml (top level, easy to find)
    if [ ! -f "$ROBIN_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ROBIN_HOME/config.yaml"
            log_success "Created ~/.robin/config.yaml from template"
        fi
    else
        log_info "~/.robin/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ROBIN_HOME/SOUL.md" ]; then
        cat > "$ROBIN_HOME/SOUL.md" << 'SOUL_EOF'
# Robin Agent Persona
...
-->
SOUL_EOF
        log_success "Created ~/.robin/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.robin/"

    # Seed bundled skills into ~/.robin/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.robin/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.robin/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ROBIN_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$ROBIN_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.robin/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Robin itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (robin --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'robin setup' after install."
        return 0
    fi
    ...
    # Run robin setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m robin_cli.main setup < /dev/tty
    else
        python -m robin_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ROBIN_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    WHATSAPP_SESSION="$ROBIN_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'robin whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ROBIN_CMD="$(get_robin_command_path)"
                $ROBIN_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'robin whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'robin gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ROBIN_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ROBIN_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: robin gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: robin gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ROBIN_CMD gateway > "$ROBIN_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.robin/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: robin gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: robin gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ROBIN_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ROBIN_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ROBIN_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}robin${NC}              Start chatting"
    echo -e "   ${GREEN}robin setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}robin config${NC}       View/edit configuration"
    echo -e "   ${GREEN}robin config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}robin gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}robin update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'robin' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'robin' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'robin' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
