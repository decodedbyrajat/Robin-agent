#!/usr/bin/env python3
"""Drive the Robin TUI under ROBIN_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.robin/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with ROBIN_TUI_RESUME set so it
bypasses the robin_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  ROBIN_PERF_LOG     (default ~/.robin/perf.log)
  ROBIN_PERF_NODE    (default node from $PATH)
  ROBIN_TUI_DIR      (default /home/bb/robin-agent/ui-tui)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests ROBIN_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(os.environ.get("ROBIN_TUI_DIR", "/home/bb/robin-agent/ui-tui"))
DEFAULT_LOG = Path(os.environ.get("ROBIN_PERF_LOG", str(Path.home() / ".robin" / "perf.log")))
DEFAULT_STATE_DB = Path.home() / ".robin" / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — ROBIN_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["ROBIN_DEV_PERF"] = "1"
    env["ROBIN_DEV_PERF_MS"] = str(args.threshold_ms)
    env["ROBIN_DEV_PERF_LOG"] = str(log)
    env["ROBIN_TUI_RESUME"] = sid
    ...
    node = os.environ.get("ROBIN_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="ROBIN_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "robin-ink" / "src"
