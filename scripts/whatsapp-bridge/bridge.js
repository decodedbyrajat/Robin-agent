 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.robin/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.robin', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.robin', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.robin', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.robin', 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '⚕ *Robin Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Robin Agent', 'Chrome', '120.0'],
