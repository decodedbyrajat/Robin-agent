...
from robin_cli.config import get_robin_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``robin dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from robin_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(robin_home: Path) -> int:
...
def _cron_summary(robin_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = robin_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"robin_home:      {display_robin_home()}")
    ...
    toolsets = config.get("toolsets", ["robin-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(robin_home)}")
    lines.append(f"  skills:             {_count_skills(robin_home)}")
