...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or robin -c.",
    ...
    # --- CLI Flags ---
    "robin -c resumes your most recent CLI session. robin -c \"project name\" resumes by title.",
    "robin -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "robin -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "robin chat -t web,terminal enables only specific toolsets for a focused session.",
    "robin chat -s github-pr-workflow preloads a skill at launch.",
    "robin chat -q \"query\" runs a single non-interactive query and exits.",
    "robin chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "robin chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "robin --yolo bypasses all dangerous command approval prompts for the entire session.",
    "robin chat --source telegram tags the session for filtering in robin sessions list.",
    "robin -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "robin doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "robin dump outputs a compact setup summary — great for bug reports.",
    "robin config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "robin config edit opens config.yaml in your default editor.",
    "robin config check scans for missing or stale configuration options.",
    "robin sessions browse opens an interactive session picker with search.",
    "robin sessions stats shows session counts by platform and database size.",
    "robin sessions prune --older-than 30 cleans up old sessions.",
    "robin skills search react --source skills-sh searches the skills.sh public directory.",
    "robin skills check scans installed hub skills for upstream updates.",
    "robin skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "robin skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "robin mcp add github --command npx adds MCP servers from the command line.",
    "robin mcp serve runs Robin itself as an MCP server for other agents.",
    "robin auth add lets you add multiple API keys for credential pool rotation.",
    "robin completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "robin logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "robin backup creates a zip backup of your entire Robin home directory.",
    "robin profile create coder creates an isolated profile that becomes its own command.",
    "robin profile create work --clone copies your current config and keys to a new profile.",
    "robin update syncs new bundled skills to ALL profiles automatically.",
    "robin gateway install sets up Robin as a system service (systemd/launchd).",
    "robin memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "robin webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: robin tools disables unused tools, robin skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "robin models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as robin -w).",
    ...
    "ROBIN_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Profile names become shell commands — 'robin profile create coder' creates the 'coder' command.",
    "robin profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "robin sessions export backup.jsonl exports all sessions for backup or analysis.",
    "robin -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "robin skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .robin/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: robin cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Robin runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "robin gateway install sets it up as a system service that starts on boot.",
    ...
    "Context files (.robin.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.robin/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Robin loads project context from .robin.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — robin tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.robin/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "robin chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "robin plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "robin acp runs Robin as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ROBIN_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "robin login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "robin status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.robin/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.robin/SOUL.md to customize.",
    ...
    "The skills audit log at ~/.robin/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at ROBIN_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "ROBIN_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ROBIN_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.robin/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'robin profile create ls' would be rejected.",
    "robin profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "robin claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ROBIN_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via robin tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.robin/.env.",
    "Skills from trusted repos get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.robin/skills/.hub/quarantine/ holds skills pending security review.",
]
