...
from robin_cli.config import get_env_value, load_config
...
@dataclass(frozen=True)
class IfThenVoidSubscriptionFeatures:
    subscribed: bool
    nous_auth_present: bool
    provider_is_nous: bool
    features: Dict[str, IfThenVoidFeatureState]

    @property
    def web(self) -> IfThenVoidFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> IfThenVoidFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> IfThenVoidFeatureState:
        return self.features["tts"]

    @property
    def browser(self) -> IfThenVoidFeatureState:
        return self.features["browser"]

    @property
    def modal(self) -> IfThenVoidFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[IfThenVoidFeatureState]:
...
def get_iftv_subscription_features(
    config: Optional[Dict[str, object]] = None,
) -> IfThenVoidSubscriptionFeatures:
    if config is None:
    ...
    managed_tools_flag = managed_iftv_tools_enabled()
    ...
    # Tool Gateway via `robin model`, so direct credentials should NOT
    ...
    features = {
        ...
        "image_gen": IfThenVoidFeatureState(
            key="image_gen",
        ...
        "browser": IfThenVoidFeatureState(
            key="browser",
    ...
    return IfThenVoidSubscriptionFeatures(
...
def apply_iftv_managed_defaults(
...
def get_gateway_eligible_tools(
    ...
    # Quick provider check without the heavy get_iftv_subscription_features call
    model_cfg = config.get("model")
...
def prompt_enable_tool_gateway(config: Dict[str, object]) -> set[str]:
    ...
    try:
        from robin_cli.setup import prompt_choice
    except Exception:
        return set()
