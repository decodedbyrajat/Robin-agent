"""``robin logs`` — view and filter Robin log files.
...
Usage examples::
    ...
    robin logs                    # last 50 lines of agent.log
    robin logs -f                 # follow agent.log in real time
    robin logs errors             # last 50 lines of errors.log
    robin logs gateway -n 100    # last 100 lines of gateway.log
    robin logs --level WARNING    # only WARNING+ lines
    robin logs --session abc123   # filter by session ID substring
    robin logs --component tools  # only tool-related lines
    robin logs --since 1h         # lines from the last hour
    robin logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_robin_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Robin runs — try 'robin chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_robin_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_robin_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_robin_home()}/logs/")
        return

    print(f"Log files in {display_robin_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'robin chat' to generate logs)")
