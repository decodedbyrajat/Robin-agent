...
Each profile is a fully independent ROBIN_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.robin/profiles/<name>/`` by default.

The "default" profile is ``~/.robin`` itself — backward compatible,
zero migration needed.

Usage::

    robin profile create coder          # fresh profile + bundled skills
    robin profile create coder --clone  # also copy config, .env, SOUL.md
    robin profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    robin -p coder chat                 # or via flag
    robin profile use coder             # set as sticky default
    robin profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See robin_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.robin`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.robin) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "robin-agent",         # repo checkout (multi-GB)
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the robin root, NOT to the current ROBIN_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ROBIN_HOME points outside
    ``~/.robin``, profiles live under ``ROBIN_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_robin_home() / "profiles"


def _get_default_robin_home() -> Path:
    """Return the default (pre-profile) ROBIN_HOME path.

    In standard deployments this is ``~/.robin``.
    In Docker/custom deployments where ROBIN_HOME is outside ``~/.robin``
    (e.g. ``/opt/data``), returns ROBIN_HOME directly.
    """
    from robin_constants import get_default_robin_root
    return get_default_robin_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_robin_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.robin
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ROBIN_HOME directory."""
    if name == "default":
        return _get_default_robin_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, robin subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _ROBIN_SUBCOMMANDS:
        return f"'{name}' conflicts with a robin subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_robin_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.robin)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.robin/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ROBIN_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ROBIN_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.robin).\n"
            "To remove everything, use: robin uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ROBIN_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ROBIN_HOME")
    try:
        os.environ["ROBIN_HOME"] = str(profile_dir)
        from robin_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ROBIN_HOME"] = old_home
        elif "ROBIN_HOME" in os.environ:
            del os.environ["ROBIN_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.robin/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: robin profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ROBIN_HOME.

    Returns ``"default"`` if ROBIN_HOME is not set or points to ``~/.robin``.
    Returns the profile name if ROBIN_HOME points into ``~/.robin/profiles/<name>``.
    Returns ``"custom"`` if ROBIN_HOME is set to an unrecognized path.
    """
    from robin_constants import get_robin_home
    robin_home = get_robin_home()
    resolved = robin_home.resolve()

    default_resolved = _get_default_robin_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.robin itself — its parent is ~/ and its
        # directory name is ".robin", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: robin profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.robin itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.robin). "
            "Specify a different name: robin profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="robin_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"robin.{old_name}"
    new_host = f"robin.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_robin_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for robin profile names."""
    return '''# Robin Agent profile completion
# Add to ~/.bashrc: eval "$(robin completion bash)"

_robin_profiles() {
    local profiles_dir="$HOME/.robin/profiles"
...
_robin_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_robin_profiles)" -- "$cur"))
        return
    fi
...
complete -F _robin_completion robin
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for robin profile names."""
    return '''#compdef robin
# Robin Agent profile completion
# Add to ~/.zshrc: eval "$(robin completion zsh)"

_robin() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.robin/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.robin/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ROBIN_HOME path string.

    Called early in the CLI entry point, before any robin modules
    are imported, to set the ROBIN_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: robin profile create {profile_name}"
        )
