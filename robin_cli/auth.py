"""
Multi-provider authentication system for Robin Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.robin/auth.json with cross-process file locking.
...
from robin_cli.config import get_robin_home, get_config_path, read_raw_config
from robin_constants import OPENROUTER_BASE_URL
...
DEFAULT_NOUS_CLIENT_ID = "robin-cli"
...
SPOTIFY_DOCS_URL = "https://github.com/IfThenVoid/Robin/tree/main/website/docs/user-guide/features/spotify"
...
def get_anthropic_key() -> str:
    ...
    """
    from robin_cli.config import get_env_value
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from robin_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.robin/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `robin model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("ROBIN_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.robin/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_robin_home() / "auth.json"
    # Seat belt: if pytest is running and ROBIN_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch ROBIN_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".robin" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set ROBIN_HOME to a tmp_path in your test fixture, or run "
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `robin logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from robin_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'robin doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'robin model' for available providers, or run 'robin doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'robin model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.robin/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("ROBIN_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.robin/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `robin auth list` / `robin status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.robin/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("ROBIN_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set ROBIN_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("ROBIN_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ROBIN_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("ROBIN_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `robin auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `robin auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `robin auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `robin auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.robin/.env, and return it.
    ...
    print("       App name:     anything (e.g. robin-agent)")
    ...
    # Persist so subsequent `robin auth spotify` runs skip the wizard.
    save_env_value("ROBIN_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("ROBIN_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved ROBIN_SPOTIFY_CLIENT_ID to ~/.robin/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "ROBIN_SPOTIFY_CLIENT_ID is required".
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.robin/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Robin auth store (~/.robin/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `robin auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `robin auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `robin auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `robin auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Robin auth store (~/.robin/auth.json)."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `robin auth` to re-authenticate."
            )
            relogin_required = True
...
def resolve_codex_runtime_credentials(
    *,
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("ROBIN_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    *,
    ...
    if "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.robin/auth.json) called POST /api/oauth/token with Robin's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Robin may call the "
            "refresh endpoint. For health checks, use `robin auth status` "
            "instead.\n"
            "Re-authenticate with: robin auth add nous"
        )
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Robin is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("ROBIN_PORTAL_BASE_URL")
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``robin auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``robin auth add nous --label <name>``).  It gets embedded in the
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Robin is not logged into Nous Portal.",
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `robin auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `robin auth` and
    # `robin model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("ROBIN_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("ROBIN_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set ROBIN_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `robin logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from robin_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from robin_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'robin model' or 'robin setup' instead."""
    print("The 'robin login' command has been removed.")
    print("Use 'robin auth' to manage credentials,")
    print("'robin model' to select a provider, or 'robin setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.robin/auth.json."""
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in ("y", "yes"):
                ...
                base_url = os.getenv("ROBIN_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Robin auth store (~/.robin/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `robin model` and select "
            "MiniMax (OAuth).",
    ...
    *,
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `robin model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `robin model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or config_matches:
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            ...
            print("Run `robin model` or configure an API key to use Robin.")
    else:
