"""
Configuration management for Robin Agent.

Config files are stored in ~/.robin/ for easy access:
- ~/.robin/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.robin/.env         - API keys and secrets

This module provides:
- robin config          - Show current configuration
- robin config edit     - Open config in editor
- robin config set      - Set a specific value
- robin config wizard   - Re-run setup wizard
"""
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `robin plugins enable
    # observability/langfuse` or `robin tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ROBIN_LANGFUSE_ENV",
    "ROBIN_LANGFUSE_RELEASE",
    "ROBIN_LANGFUSE_SAMPLE_RATE",
    "ROBIN_LANGFUSE_MAX_CHARS",
    "ROBIN_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from robin_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ROBIN_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_robin_home() / ".managed"
...
def is_managed() -> bool:
    ...
    Two signals: the ROBIN_MANAGED env var (set by the systemd service),
    or a .managed marker file in ROBIN_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade robin-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "robin update"


def format_managed_message(action: str = "modify this Robin installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ROBIN_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Robin installation is managed by NixOS "
            f"(ROBIN_MANAGED={env_hint}).\n"
            "Edit services.robin-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ROBIN_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, robin_bin
    or None if container mode is not active, we're already inside the
    container, or ROBIN_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ROBIN_DEV") == "1":
        return None
    ...
    container_mode_file = get_robin_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "robin-agent")
    exec_user = info.get("exec_user", "robin")
    robin_bin = info.get("robin_bin", "/data/current-package/bin/robin")
...
# Re-export from robin_constants — canonical definition lives there.
from robin_constants import get_robin_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_robin_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_robin_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the robin group can
    share state with the gateway service.

    The mode can be overridden via the ROBIN_HOME_MODE environment variable
    (e.g. ROBIN_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ROBIN_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ROBIN_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("ROBIN_CONTAINER") or os.environ.get("ROBIN_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ROBIN_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ROBIN_HOME if the user doesn't have one yet."""
...
def ensure_robin_home():
    """Ensure ~/.robin directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_robin_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_robin_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ROBIN_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        #  "/home/user/.robin/cache/documents:/output"]
        ...
        # bundled Robin image, which drops to the `robin` user via gosu).
    ...
    "checkpoints": {
        ...
        # ~/.robin/checkpoints/ (one per cd'd working directory). Field
        # reports put the typical offender at 1000+ repos / ~12 GB. When
        # auto_prune is on, robin sweeps at startup (at most once per
        # min_interval_hours) and deletes:
        #   * orphan repos: ROBIN_WORKDIR no longer exists on disk
    ...
    "auxiliary": {
        ...
        # model; override via `robin model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `robin --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `robin -c` muscle memory.  Default off so existing
        # users aren't surprised.  ROBIN_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/robin`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.robin/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.robin/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.robin/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ROBIN_SKILL_DIR} and ${ROBIN_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `robin curator status` for the last run summary.
    "curator": {
        "enabled": True,
    ...
    # This section is only needed for robin-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Robin Agent*" header.
    ...
    # stored approval from ~/.robin/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ROBIN_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via ROBIN_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     robin-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.robin/logs/.
    ...
    # update model picker lists without shipping a robin-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/IfThenVoid/Robin/tree/main/website/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `robin model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.robin/state.db.
    ...
    "sessions": {
        ...
        # default of ``robin sessions prune``.
        "retention_days": 90,
    ...
    # ``robin update`` behaviour.
    "updates": {
        # Run a full ``robin backup``-style zip of ROBIN_HOME before every
        # ``robin update``.  Backups land in ``<ROBIN_HOME>/backups/`` and
        # can be restored with ``robin import <path>``.  Off by default —
        # on large ROBIN_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'robin model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Robin auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @robin:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ROBIN_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #robin)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: robin-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'robin-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # ROBIN_TOOL_PROGRESS and ROBIN_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.robin.config`` entries, then checks
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``robin_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'robin setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'robin doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ROBIN_HOME", "~/.robin")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ROBIN_TOOL_PROGRESS")
            old_mode = get_env_value("ROBIN_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ROBIN_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ROBIN_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ROBIN_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ROBIN_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ROBIN_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_robin_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `robin plugins enable <name>` to activate."
                    )
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: robin config set <key> <value>")
    ...
    # metadata.robin.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _robin_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.robin/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.robin/config.yaml.
    ...
    (which change ``ROBIN_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    ensure_robin_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — robin auth) — OpenAI Codex
#   nous         (OAuth — robin auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.robin/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.robin/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.robin/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.robin/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.robin/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.robin/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.robin/.env or environment."""
...
def show_config():
    ...
    print(color("  robin config edit     # Edit config file", Colors.DIM))
    print(color("  robin config set <key> <value>", Colors.DIM))
    print(color("  robin setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: robin config set <key> <value>")
            print()
            print("Examples:")
            print("  robin config set model anthropic/claude-sonnet-4")
            print("  robin config set terminal.backend docker")
            print("  robin config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'robin config migrate' to add them")
        ...
        print("  robin config           Show current configuration")
        print("  robin config edit      Open config in editor")
        print("  robin config set <key> <value>   Set a config value")
        print("  robin config check     Check for missing/outdated config")
        print("  robin config migrate   Update config with new options")
        print("  robin config path      Show config file path")
        print("  robin config env-path  Show .env file path")
        sys.exit(1)
