"""robin webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    robin webhook subscribe <name> [options]
    robin webhook list
    robin webhook remove <name>
    robin webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.robin/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _robin_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: robin gateway run
"""
...
def webhook_command(args):
    """Entry point for 'robin webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: robin webhook {subscribe|list|remove|test}")
        print("Run 'robin webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (robin gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: robin webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from robin webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (robin gateway run)")
