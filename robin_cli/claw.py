"""robin claw — OpenClaw migration commands.

Usage:
    robin claw migrate              # Preview then migrate (always shows preview first)
    robin claw migrate --dry-run    # Preview only, no changes
    robin claw migrate --yes        # Skip confirmation prompt
    robin claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    robin claw migrate --no-backup  # Skip pre-migration snapshot
    robin claw cleanup              # Archive leftover OpenClaw directories
    robin claw cleanup --dry-run    # Preview what would be archived
"""
...
from robin_cli.config import get_robin_home, get_config_path, load_config, save_config
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print_info("Recommendation: stop the gateway first with 'robin stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_robin.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_robin", script_path)
...
def _cmd_migrate(args):
    ...
    # migrate-robin posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: robin claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {robin_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.robin/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: robin claw migrate --yes")
            return
    ...
    if not no_backup:
        try:
            from robin_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(robin_home=robin_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: robin import {backup_archive.name}")
        except Exception as e:
            print()
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: robin claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: robin claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  robin claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  robin claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  robin config set OPENROUTER_API_KEY sk-or-v1-...")
