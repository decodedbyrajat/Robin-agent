"""
Unified tool configuration for Robin Agent.

`robin tools` and `robin setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.robin/config.yaml under
...
from robin_cli.iftv_subscription import (
    apply_iftv_managed_defaults,
    get_iftv_subscription_features,
)
from tools.tool_backend_helpers import fal_key_is_configured, managed_iftv_tools_enabled
...
from robin_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ROBIN_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl", "spotify", "discord", "discord_admin"}

# Platform-scoped toolsets: only appear in the `robin tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``robin tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from robin_cli.plugins import discover_plugins, get_plugin_toolsets
...
from robin_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ROBIN_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ROBIN_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ROBIN_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ROBIN_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ROBIN_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_robin_home()}/robin-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in robin-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/ifthentvoid/robin-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.robin/config.yaml")

    elif post_setup_key == "spotify":
        # Run the full `robin auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.robin/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from robin_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: robin auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `robin auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: robin auth spotify")
        except Exception as exc:
        ...
        try:
            from robin_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: robin plugins enable observability/langfuse")
        _print_info("    Restart Robin for tracing to take effect.")
        _print_info("    Verify: robin plugins list")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "robin-cli" (which include all _ROBIN_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "robin-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # otherwise saving via `robin tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"robin-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("robin-")}
    ...
    # shipped as a bundled plugin but user must opt in via `robin tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `robin tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by robin tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (robin-cli, robin-telegram, etc.)
    ...
    # Opening `robin tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(ts_key: str, config: dict = None) -> bool:
    ...
    if ts_key in {"web", "image_gen", "tts", "browser"}:
        features = get_iftv_subscription_features(config)
...
def _visible_providers(cat: dict, config: dict) -> list[dict]:
    ...
    for provider in cat.get("providers", []):
        if provider.get("managed_nous_feature") and not managed_iftv_tools_enabled():
            continue
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `robin tools` and `robin setup tools`.
    ...
    print(color("  Guide: https://github.com/IfThenVoid/Robin/tree/main/website/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_iftv_managed_defaults(
                config,
    ...
    print()
    from robin_constants import display_robin_home
    print(color(f"  Tool configuration saved to {display_robin_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'robin' or gateway restart.", Colors.DIM))
    print()
