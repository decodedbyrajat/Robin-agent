"""CLI subcommand: `robin curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not args.synchronous:
        print("llm pass running in background — check `robin curator status` later")
    return 0
...
# ---------------------------------------------------------------------------
# argparse wiring (called from robin_cli.main)
# ---------------------------------------------------------------------------
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by robin_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="robin curator")
    register_cli(parser)
