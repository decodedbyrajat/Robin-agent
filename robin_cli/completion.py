"""Shell completion script generation for robin CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_robin_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Robin Agent bash completion
# Add to ~/.bashrc:
#   eval "$(robin completion bash)"
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef robin
# Robin Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(robin completion zsh)"
...
_robin() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_robin_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'robin command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Robin Agent fish completion",
        "# Add to your config:",
        "#   robin completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __robin_profiles",
        "    echo default",
        "    if test -d $HOME/.robin/profiles",
        "        ls $HOME/.robin/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c robin -f -s p -l profile"
        " -d 'Profile name' -xa '(__robin_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__robin_profiles)' -d 'Profile name'"
                )
