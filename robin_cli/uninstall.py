"""
Robin Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.robin/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing robin-agent or robin PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Robin Agent" comment and following line
                if '# Robin Agent' in line or '# robin-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('robin' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing robin
                if 'robin' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the robin wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "robin",
        Path("/usr/local/bin/robin"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains robin_cli reference)
                content = wrapper.read_text()
                if 'robin_cli' in content or 'robin-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``robin gateway run`` processes
    ...
    try:
        from robin_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from robin_cli.gateway import get_launchd_plist_path
...
def _is_default_robin_home(robin_home: Path) -> bool:
    """Return True when ``robin_home`` points at the default (non-profile) root."""
    try:
        from robin_constants import get_default_robin_root
        return robin_home.resolve() == get_default_robin_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ROBIN_HOME directory.

    We shell out to ``robin -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ROBIN_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m robin_cli.main` so we don't depend on a `robin`
    #    wrapper that may be half-removed mid-uninstall.
    robin_invocation = [_sys.executable, "-m", "robin_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                robin_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ROBIN_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.robin/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.robin/ for future reinstall
    """
    project_root = get_project_root()
    robin_home = get_robin_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ROBIN_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_robin_home(robin_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Robin Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {robin_home / 'config.yaml'}")
    print(f"  Secrets: {robin_home / '.env'}")
    print(f"  Data:    {robin_home / 'cron/'}, {robin_home / 'sessions/'}, {robin_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ROBIN_HOME dirs. Otherwise
    ...
    log_info("Removing robin command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.robin/, just remove the robin-agent subdir
            if robin_home in project_root.parents or project_root.parent == robin_home:
    ...
    # 5. Optionally remove ~/.robin/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ROBIN_HOME dirs live under
        ...
        try:
            if robin_home.exists():
                shutil.rmtree(robin_home)
                log_success(f"Removed {robin_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {robin_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {robin_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/IfThenVoid/Robin/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using Robin Agent! ⚕")
    print()
