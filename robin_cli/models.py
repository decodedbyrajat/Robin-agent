...
Add, remove, or reorder entries here — both `robin setup` and
`robin` provider-selection will pick up the change automatically.
"""
...
_ROBIN_USER_AGENT = f"robin-cli/{_ROBIN_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `robin model`
    flow without maintaining a separate static list.
    """
    from robin_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $ROBIN_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ROBIN_HOME/models_dev_cache.json directly (no network) so this
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from robin_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  robin model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `robin model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `robin model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "Nous Portal",              "Nous Portal subscription"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``robin model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``robin auth add openai-codex`` without
    ``robin model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    robin-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from robin_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — robin-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from robin_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from robin_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return robin-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; robin's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``robin model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from robin_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from robin_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``robin_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``robin auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.robin/.env``.
    ...
    """
    try:
        from robin_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from robin_cli.auth import read_credential_pool
        from robin_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from robin_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ROBIN_USER_AGENT}
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ROBIN_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_robin_home() / "ollama_cloud_models_cache.json"
