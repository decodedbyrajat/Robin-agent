...
process replacement so that ``robin sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``robin`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched robin."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_robin_bin() -> Optional[str]:
    """Find the robin entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("robin")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m robin_cli.main``.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with robin.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_robin_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "robin_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh robin invocation."""
