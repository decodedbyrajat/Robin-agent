...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with robin-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.robin/plugins/<name>/``
3. **Project plugins** – ``./.robin/plugins/<name>/`` (opt-in via
   ``ROBIN_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``robin_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "robin_agent.plugins"

_NS_PARENT = "robin_plugins"
...
class PluginManifest:
    ...
    #              in ~/.robin/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``robin plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``robin honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``robin <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.robin/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to robin_cli/. Two layouts are
        ...
        # 2. User plugins (~/.robin/plugins/)
        user_dir = get_robin_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.robin/plugins/)
        if _env_enabled("ROBIN_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".robin" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with robin and must
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `robin plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``robin_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``robin_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``robin tools`` TUI so plugin-provided toolsets appear
