...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.robin/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Robin Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`robin_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/robin`` subcommand mapping, and
    ...
    """
    try:
        from robin_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``robin skills
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/robin <verb>`` subcommand.
    ...
    avoidance. ``/robin`` is always reserved as the first entry so the
    legacy ``/robin <subcommand>`` form keeps working for anything that
    ...
    # Reserve /robin as the catch-all top-level command.
    entries.append(("robin", "Talk to Robin or run a subcommand", "[subcommand] [args]"))
    seen.add("robin")
...
def slack_app_manifest(request_url: str = "https://robin-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /robin handler.

    Maps both canonical names and aliases so /robin bg do stuff works
    the same as /robin background do stuff.

    Plugin-registered slash commands are included so ``/robin <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from robin_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
