"""``robin slack ...`` CLI subcommands.

Today only ``robin slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ robin slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    pasting.
    """
    from robin_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``robin_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$ROBIN_HOME/slack-manifest.json``)
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path.home() / ".robin" / "slack-manifest.json"
        else:
