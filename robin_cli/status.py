"""
Status command for robin CLI.

Shows the status of all Robin Agent components.
"""
...
from robin_cli.auth import AuthError, resolve_provider
from robin_cli.colors import Colors, color
from robin_cli.config import get_env_path, get_env_value, get_robin_home, load_config
from robin_cli.models import provider_label
from robin_cli.iftv_subscription import get_iftv_subscription_features
from robin_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``robin config``'s
...
def show_status(args):
    """Show status of all Robin Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Robin Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: robin auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: robin model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: robin auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: robin model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from robin_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'robin-agent[vercel]')"
    ...
    try:
        from robin_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   robin gateway")
    ...
    jobs_file = get_robin_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_robin_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'robin doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'robin setup' to configure", Colors.DIM))
    print()
