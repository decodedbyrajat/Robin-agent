"""
Backup and import commands for robin CLI.

`robin backup` creates a zip archive of the entire ~/.robin/ directory
(excluding the robin-agent repo and transient files).

`robin import` restores from a backup zip, overlaying onto the current
ROBIN_HOME root.
"""
...
from robin_constants import get_default_robin_root, get_robin_home, display_robin_home
...
_EXCLUDED_DIRS = {
    "robin-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to robin root) should be skipped."""
...
def run_backup(args) -> None:
    ...
    robin_root = get_default_robin_root()

    if not robin_root.is_dir():
        print(f"Error: Robin home directory not found at {robin_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"robin-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"robin-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_robin_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(robin_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(robin_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(robin_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside robin root
            try:
    ...
    print(f"\nRestore with: robin import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a robin home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.robin/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a robin dir name
        if prefix in (".robin", "robin"):
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_robin_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (robin_root / "config.yaml").exists()
        has_env = (robin_root / ".env").exists()
        ...
        for member in members:
            ...
            target = robin_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(robin_root.resolve())
            except ValueError:
        ...
        profiles_dir = robin_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # robin_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: robin profile list  (after installing robin)")

        # Guidance
        print()
        if not (robin_root / "robin-agent").is_dir():
            print("Note: The robin-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: robin update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  robin -p {pname} gateway install")

        print("Done. Your Robin configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and robin backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ROBIN_HOME).
...
# — `robin update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(robin_home: Optional[Path] = None) -> Path:
    home = robin_home or get_robin_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    robin_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(robin_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(robin_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for robin backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_robin_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, robin_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``robin_root`` to ``out_path``.
...
def _pre_update_backup_dir(robin_home: Optional[Path] = None) -> Path:
    home = robin_home or get_robin_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ROBIN_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ROBIN_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``robin update``) should continue even if the backup fails.
    """
    robin_root = robin_home or get_default_robin_root()
    if not robin_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(robin_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, robin_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `robin claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ROBIN_HOME under ``backups/`` before a
    ``robin claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``robin import <archive>``.  Writes to
    ``<ROBIN_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `robin import` and the
