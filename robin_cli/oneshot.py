...
configured for "cli" in `robin tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (ROBIN_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `robin chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    ...
    - ROBIN_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"robin -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "robin -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"robin -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "robin -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "robin -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to ROBIN_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            ROBIN_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
    ...
    env_model_early = os.getenv("ROBIN_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "robin -z: --provider requires --model (or ROBIN_INFERENCE_MODEL). "
    ...
    os.environ["ROBIN_ACCEPT_HOOKS"] = "1"
...
def _run_agent(
    ...
    # Imports are local so they don't run when robin is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from robin_cli.config import load_config
    from robin_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("ROBIN_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from robin_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("ROBIN_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                ROBIN_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via ROBIN_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via ROBIN_YOLO_MODE=1
