...
- ``robin_cli.providers``        -- canonical provider identity + overlays
- ``robin_cli.model_normalize``  -- per-provider name formatting
...
_ROBIN_MODEL_WARNING = (
    "Robin 3 & 4 models are NOT agentic and are not designed "
    "for use with Robin Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Robin 3 / Robin 4 chat families.
# The previous substring check (`"robin" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``robin-brain:qwen3-14b-ctx16k`` that just
# happen to carry "robin" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Robin-3-Llama-3.1-70B, robin-4-405b, openrouter/robin3:70b
# Negative examples it must NOT match:
#   robin-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ROBIN_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])robin[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_robin_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Robin 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ROBIN_NON_AGENTIC_RE.search(model_name))


def _check_robin_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Robin 3/4 chat model."""
    if is_nous_robin_non_agentic(model_name):
        return _ROBIN_MODEL_WARNING
    return ""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from robin_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'robin model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'robin doctor' — config issues detected:"
    ...
    # Mirrors the same logic in robin_cli.runtime_provider.resolve_runtime_provider;
    ...
    robin_warn = _check_robin_model_warning(new_model)
    if robin_warn:
        warnings.append(robin_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from robin_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from robin_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by robin provider ID
    ...
    for robin_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(robin_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ROBIN_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from robin_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and robin_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Robin release.
        model_ids = curated.get(robin_id, [])
        if robin_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(robin_id, model_ids)
        total = len(model_ids)
    ...
    from robin_cli.providers import ROBIN_OVERLAYS
    from robin_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Robin provider ID.
    # ROBIN_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Robin IDs ("copilot").
    _mdev_to_robin = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ROBIN_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Robin slug — e.g. "github-copilot" → "copilot"
        robin_slug = _mdev_to_robin.get(pid, pid)
        if robin_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or robin_slug in providers_store
                    or pid in pool_store or robin_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(robin_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", robin_slug, exc)
        ...
        # configured.
        if not has_creds and robin_slug == "anthropic":
            try:
                ...
                if (robin_creds and robin_creds.get("accessToken")) or \
        ...
        if robin_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(robin_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(robin_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(robin_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Robin slug, fall back to overlay key
            model_ids = curated.get(robin_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if robin_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(robin_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": robin_slug,
            "name": get_label(robin_slug),
            "is_current": robin_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "robin",
        ...
        seen_slugs.add(robin_slug.lower())
        _record_builtin_endpoint(robin_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ROBIN_OVERLAYS (keeps /model in sync
    # with `robin model`).
    try:
        from robin_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # (see robin_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, robin_cli/config.py) already consume that dict.
