"""``robin plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.robin/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    plugins = get_robin_home() / "plugins"
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from robin_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_robin_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_robin_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_robin_home()}/.env later)[/dim]")
...
def cmd_install(
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]robin plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `robin plugins enable {installed_name}` to activate.[/dim]"
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  robin gateway restart[/dim]")
    console.print()
    ...
    if user_dir.is_dir():
        ...
        for child in user_dir.iterdir():
            ...
            if manifest.get("name") == name:
                return True
    # Bundled: <repo>/plugins/<name>/ (or ROBIN_BUNDLED_PLUGINS on Nix).
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] robin plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] robin plugins")
    console.print("[dim]Enable/disable:[/dim] robin plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch robin plugins subcommands."""
