...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_robin_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:robin_pkce",
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `robin auth
    ...
    raise SystemExit(f"`robin auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `robin auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `robin auth` is called bare."""
