...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.robin/cache/model_catalog.json``.
...
def _cache_path() -> Path:
    ...
    return get_robin_home() / "cache" / "model_catalog.json"
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``robin model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
