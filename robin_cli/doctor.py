"""
Doctor command for robin CLI.

Diagnoses issues with Robin Agent setup.
"""
...
from robin_cli.config import get_project_root, get_robin_home, get_env_path
from robin_constants import display_robin_home

PROJECT_ROOT = get_project_root()
ROBIN_HOME = get_robin_home()
_DHH = display_robin_home()  # user-facing display path (e.g. ~/.robin or ~/.robin/profiles/coder)

# Load environment variables from ~/.robin/.env so API key checks work
...
from robin_cli.models import _ROBIN_USER_AGENT
from robin_cli.vercel_auth import describe_vercel_auth
from robin_constants import OPENROUTER_MODELS_URL
...
from robin_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.robin/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `robin`.
    os.environ.setdefault("ROBIN_INTERACTIVE", "1")
    ...
    # Check ~/.robin/.env (primary location for user config)
    env_path = ROBIN_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'robin setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'robin setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'robin setup' to create one")
                issues.append("Run 'robin setup' to create .env")
    
    # Check ~/.robin/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from robin_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from robin_cli.providers import (
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'robin config set model.provider <valid_provider>'"
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom", "openrouter"):
                try:
                    from robin_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.robin/.env or run 'robin setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'robin setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'robin config set model.provider <name>'"
                            )
                except Exception:
                    pass
    ...
    if config_path.exists():
        try:
            from robin_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'robin setup' to migrate config")
                else:
                    issues.append("Run 'robin doctor --fix' or 'robin setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'robin doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # `robin auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    if robin_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            robin_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = robin_home / subdir_name
    ...
    memories_dir = robin_home / "memories"
    ...
    state_db_path = robin_home / "state.db"
    ...
    wal_path = robin_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'robin doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (robin bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "robin"
        ...
        _cmd_link = _cmd_link_dir / "robin"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(robin not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/robin → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/robin points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/robin → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/robin — run 'robin doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/robin exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/robin not found",
                    "(robin command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/robin → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/robin symlink — run 'robin doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'robin-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'robin-agent[vercel]'")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                issues.append(
                    ...
                    "Fix: run 'robin config set model.provider <provider>' to switch providers, "
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'robin setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: robin skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = ROBIN_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: robin memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: robin memory setup")
                issues.append("No Honcho API key — run 'robin memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run robin memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: robin memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: robin memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from robin_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "robin -p" in content:
                            _m = _re.search(r"robin -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'robin doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
