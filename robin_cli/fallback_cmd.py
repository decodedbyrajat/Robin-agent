"""
robin fallback — manage the fallback provider chain.
...
Subcommands:
  robin fallback [list]   Show the current fallback chain (default when no subcommand)
  robin fallback add      Pick provider + model via the same picker as `robin model`,
                           then append the selection to the chain
  robin fallback remove   Pick an entry to delete from the chain
  robin fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.robin/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from robin_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `robin model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  robin fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/IfThenVoid/Robin/tree/main/website/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `robin model`, then append the selection to the chain."""
    from robin_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `robin model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `robin fallback list` to view, or `robin fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``robin fallback [subcommand]``."""
