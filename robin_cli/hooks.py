"""robin hooks — inspect and manage shell-script hooks.

Usage::

    robin hooks list
    robin hooks test <event> [--for-tool X] [--payload-file F]
    robin hooks revoke <command>
    robin hooks doctor

Consent records live under ``~/.robin/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.robin/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``robin hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: robin hooks {list|test|revoke|doctor}")
        print("Run 'robin hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.robin/config.yaml.")
        print("See `robin hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `robin hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `robin hooks test` and `robin hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `robin hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `robin hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `robin hooks doctor`.")
