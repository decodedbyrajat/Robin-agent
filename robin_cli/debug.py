"""``robin debug`` — debug tools for Robin Agent.

Currently supports:
    robin debug share    Upload debug report (system info + logs) to a
...
def _pending_file() -> Path:
    """Path to ``~/.robin/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``robin debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``robin debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `robin debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "robin-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``robin debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.robin/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``robin debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"robin debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "robin-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from robin_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_robin_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``robin dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``robin dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    print(f"To delete now:  robin debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: robin debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'robin debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``robin debug`` call.
    ...
    # best-effort — any failure is swallowed so ``robin debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: robin debug <command>")
        print()
        print("Commands:")
