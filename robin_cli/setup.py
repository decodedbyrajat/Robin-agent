"""
Interactive setup wizard for Robin Agent.
...
from robin_cli.iftv_subscription import get_iftv_subscription_features
...
_DOCS_BASE = "https://github.com/IfThenVoid/Robin/tree/main/website/docs"
...
# display_robin_home imported lazily at call sites (stale-module safety during robin update)
...
from robin_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("  robin config set model.provider custom")
    print_info("  robin config set model.base_url http://localhost:8080/v1")
    print_info("  robin config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'robin setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from robin_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from robin_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'robin setup')")


def _print_setup_summary(config: dict, robin_home):
    ...
    tool_status = []
    subscription_features = get_iftv_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'robin setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from robin_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'robin setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'robin setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'robin setup terminal'"))
    elif managed_iftv_tools_enabled() and subscription_features.nous_auth_present:
    ...
    # Spotify (OAuth via robin auth spotify — check auth.json, not env vars)
    try:
        from robin_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'robin setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {robin_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('robin setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('robin setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('robin setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('robin setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('robin setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('robin config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('robin config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('robin config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('robin', Colors.GREEN)}              Start chatting")
    print(f"   {color('robin gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('robin doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `robin tools` and `robin setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``robin model``)
    ...
    provider added to ``robin model`` is automatically available here.
    ...
    # Delegate to the shared robin model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from robin_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: robin model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from robin_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Robin can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Robin can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'robin setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_iftv_tools_enabled() and subscription_features.nous_auth_present:
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.robin/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"robin setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'robin setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Robin via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("ROBIN_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `robin setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("ROBIN_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("ROBIN_MAX_ITERATIONS", str(max_iter))
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `robin update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://github.com/IfThenVoid/Robin/tree/main/website/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ROBIN_HOME, and print
    ...
    """
    try:
        from robin_cli.slack_cli import _build_full_manifest
        ...
        target = Path(get_robin_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `robin slack manifest --write` anytime to refresh after "
...
def _setup_mattermost():
    ...
    print_info("   Open config in your editor:  robin config edit")
...
def _setup_qqbot():
    ...
    from robin_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://github.com/IfThenVoid/Robin/tree/main/website/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://github.com/IfThenVoid/Robin/tree/main/website/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  robin config edit")
    print_info("   Open config in your editor:  robin config edit")
...
def setup_gateway(config: dict):
    ...
    from robin_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if not selected:
        print_info("No platforms selected. Run 'robin setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     robin config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_robin_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  robin gateway")
        else:
            from robin_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   robin gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   robin gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `robin setup tools` and `robin tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``robin_cli.auth`` — lists every supported
    ...
    """
    try:
        from robin_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from robin_cli.config
    ...
    if section_key == "model":
        ...
        from robin_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_robin.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_robin migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_robin", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(robin_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: robin claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=robin_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: robin claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Robin (use robin claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      robin setup           — full or quick (auto-detected)
      robin setup model     — just model/provider
      robin setup tts       — just text-to-speech
      robin setup terminal  — just terminal backend
      robin setup gateway   — just messaging platforms
      robin setup tools     — just tool configuration
      robin setup agent     — just agent settings
    """
    ...
    print(
        color(
            "│             ⚕ Robin Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Robin Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, robin_home)
            return
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'robin setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(robin_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Robin?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, robin_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {robin_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'robin config edit'")
    ...
    save_config(config)
    _print_setup_summary(config, robin_home)
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch robin chat now?", True):
        return
...
def _run_first_time_quick_setup(config: dict, robin_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'robin setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    robin setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  robin setup gateway")
    print()
...
def _run_quick_setup(config: dict, robin_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'robin setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'robin setup gateway'.")
