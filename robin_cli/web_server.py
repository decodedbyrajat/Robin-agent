"""
Robin Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m robin_cli.main web          # Start on http://127.0.0.1:9119
    python -m robin_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["ROBIN_WEB_DIST"]) if "ROBIN_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Robin Agent", version=__version__)
...
# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``robin dashboard --tui``
# or ROBIN_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
_PUBLIC_API_PATHS: frozenset = frozenset({
    ...
    "/api/voice/transact",  # Phase 9: Tauri Voice Shell — no browser session available
})
...
# streamed to a per-action log file under ``~/.robin/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_robin_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "robin-update": "robin-update.log",
}
...
def _spawn_robin_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``robin <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``robin_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "robin_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ROBIN_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``robin gateway restart`` in the background."""
    try:
        proc = _spawn_robin_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/robin/update")
async def update_robin():
    """Kick off ``robin update`` in the background."""
    try:
        proc = _spawn_robin_action(["update"], "robin-update")
    except Exception as exc:
        _log.exception("Failed to spawn robin update")
...
@app.post("/api/voice/transact")
async def voice_transact(audio: UploadFile = File(...)):
    """MVP Phase 9 Voice Endpoint: Receives audio, runs STT -> LLM -> TTS, returns audio."""
    import asyncio
    ...
    try:
        ...
        # 2. Transcribe (blocking → run in executor)
        def _transcribe():
            from tools.transcription_tools import transcribe_audio
            result = transcribe_audio(tmp_path)
            return result

        _log.info("Voice transact: transcribing %s", tmp_path)
        stt_result = await loop.run_in_executor(None, _transcribe)

        if not isinstance(stt_result, dict) or not stt_result.get("success"):
            err = stt_result.get("error", "Unknown error") if isinstance(stt_result, dict) else str(stt_result)
            raise HTTPException(status_code=500, detail=f"Transcription failed: {err}")

        transcript = stt_result.get("transcript", "").strip()
        if not transcript:
            raise HTTPException(status_code=400, detail="Empty transcript — nothing was heard.")

        _log.info("Voice transact: heard: %r", transcript)

        # 3. Run through LLM agent (blocking → run in executor)
        def _think():
            ...
            result = agent.run_conversation(transcript)
            # run_conversation returns the last assistant text turn
            if isinstance(result, str):
                return result
            # Some callers return a messages list — extract last assistant turn
        ...
        _log.info("Voice transact: thinking...")
        response_text = await loop.run_in_executor(None, _think)
        if not response_text:
            raise HTTPException(status_code=500, detail="LLM returned empty response.")

        _log.info("Voice transact: response: %r", response_text[:80])

        # 4. TTS (blocking → run in executor)
        def _tts():
            from tools.tts_tool import text_to_speech_tool
            tts_json = text_to_speech_tool(response_text)
            tts_res = json.loads(tts_json)
            return tts_res

        _log.info("Voice transact: generating TTS...")
        tts_res = await loop.run_in_executor(None, _tts)

        if not tts_res.get("success"):
            raise HTTPException(status_code=500, detail=f"TTS failed: {tts_res.get('error')}")

        tts_audio_path = tts_res.get("file_path")
        if not tts_audio_path or not os.path.exists(tts_audio_path):
            raise HTTPException(status_code=500, detail="TTS produced no audio file.")

        _log.info("Voice transact: returning audio %s", tts_audio_path)

        # Use background task to clean up after response is sent
        from fastapi.background import BackgroundTask

        def _cleanup():
            for p in [tmp_path, tts_audio_path]:
                try:
        ...
        # Detect audio format from extension and set correct mime type
        tts_ext = os.path.splitext(tts_audio_path)[1].lower()
        mime_map = {".mp3": "audio/mpeg", ".ogg": "audio/ogg", ".opus": "audio/ogg",
                    ".wav": "audio/wav", ".m4a": "audio/mp4"}
        mime = mime_map.get(tts_ext, "audio/mpeg")

        return FileResponse(tts_audio_path, media_type=mime, background=BackgroundTask(_cleanup))

    except HTTPException:
        # Clean up tmp_path on error
        try:
            os.unlink(tmp_path)
        except OSError:
            pass
        ...
        _log.exception("Voice transaction failed")
        try:
            os.unlink(tmp_path)
        except OSError:
            pass
...
# in robin_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.robin/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``robin auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.robin/.anthropic_oauth.json`` — Robin-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_robin_oauth_credentials,
            read_claude_code_credentials,
            _ROBIN_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_robin_oauth_credentials = None  # type: ignore
        _ROBIN_OAUTH_FILE = None  # type: ignore

    robin_creds = None
    if read_robin_oauth_credentials:
        try:
            robin_creds = read_robin_oauth_credentials()
        except Exception:
            robin_creds = None
    if robin_creds and robin_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "robin_pkce",
            "source_label": f"Robin PKCE ({_ROBIN_OAUTH_FILE})",
            "token_preview": _truncate_token(robin_creds.get("accessToken")),
            "expires_at": robin_creds.get("expiresAt"),
            "has_refresh_token": bool(robin_creds.get("refreshToken")),
        }
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "robin auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "robin auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "robin auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "robin auth add qwen-oauth",
    ...
    },
    {
        ...
        "name": "MiniMax (OAuth)",
        "flow": "pkce",
        "cli_command": "robin auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from robin_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("robin_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _ROBIN_OAUTH_FILE
            if _ROBIN_OAUTH_FILE.exists():
                _ROBIN_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from robin_cli.auth import clear_provider_auth
...
#          → persists to ~/.robin/.anthropic_oauth.json AND credential pool
...
# Guarded so robin web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    the system in the same state as ``robin auth add anthropic``.
    """
    from agent.anthropic_adapter import _ROBIN_OAUTH_FILE
    payload = {
    ...
    _ROBIN_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _ROBIN_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "robin-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from robin_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("ROBIN_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from robin_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from robin_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = (
            os.getenv("ROBIN_CODEX_BASE_URL", "").strip().rstrip("/")
...
async def get_logs(
    ...
    from robin_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_robin_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from robin_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from robin_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from robin_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from robin_cli.tools_config import (
...
#
# The endpoint spawns the same ``robin --tui`` binary the CLI uses, behind
...
from robin_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``robin --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ROBIN_TUI_RESUME`` env var —
    matching what ``robin_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``ROBIN_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from robin_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if resume:
        env["ROBIN_TUI_RESUME"] = resume

    if sidecar_url:
        env["ROBIN_TUI_SIDECAR_URL"] = sidecar_url
...
# ROBIN_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__ROBIN_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__ROBIN_DASHBOARD_EMBEDDED_CHAT__={chat_js};</script>"
        )
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "Robin Teal",  "description": "Classic dark teal — the canonical Robin look"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.robin/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.robin/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_robin_home() / "dashboard-themes"
...
async def get_system_health():
    """Return Mac system metrics for the monitoring dashboard."""
    ...
    try:
        ...
        result["cpu_percent"] = _psutil.cpu_percent(interval=0.3)
        ...
        result["uptime_seconds"] = int(_time.time() - _psutil.boot_time())
    except Exception as exc:
        result["psutil_error"] = str(exc)

    # --- Mac thermal pressure via sysctl ---
    try:
        r = _sp.run(
            ["sysctl", "-n", "kern.thermalStatus"],
            capture_output=True, text=True, timeout=2
        )
        result["thermal_status"] = int(r.stdout.strip()) if r.stdout.strip().isdigit() else None
    except Exception:
        result["thermal_status"] = None

    # --- Ollama reachability ---
    try:
        import urllib.request as _req
        with _req.urlopen("http://localhost:11434/api/tags", timeout=2) as resp:
            tags = __import__("json").loads(resp.read())
            result["ollama_running"] = True
            result["ollama_models"] = [m["name"] for m in tags.get("models", [])]
    except Exception:
...
async def get_dashboard_themes():
    ...
    from `~/.robin/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as robin_cli.plugins):
    1. User plugins:    ~/.robin/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.robin/plugins/  (only if ROBIN_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from robin_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_robin_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("ROBIN_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".robin" / "plugins", "project"))
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"robin_dashboard_plugin_{plugin['name']}"
