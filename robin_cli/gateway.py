"""
Gateway subcommand for robin CLI.

Handles: robin gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_robin_home is imported lazily at call sites to avoid ImportError
# when robin_constants is cached from a pre-update version during `robin update`.
from robin_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from robin_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "robin-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "robin_cli.main gateway",
        "robin_cli.main --profile",
        "robin_cli.main -p",
        "robin_cli/main.py gateway",
        "robin_cli/main.py --profile",
        "robin_cli/main.py -p",
        "robin gateway",
        "gateway/run.py",
    ]
    current_home = str(get_robin_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"ROBIN_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "ROBIN_HOME=" in command and f"ROBIN_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``robin update``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}robin gateway status\n"
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from robin_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `robin gateway`")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ROBIN_HOME-scoped).
...
from robin_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "robin-gateway"
SERVICE_DESCRIPTION = "Robin Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ROBIN_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ROBIN_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from robin_constants import get_default_robin_root
    home = get_robin_home().resolve()
    default = get_default_robin_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ROBIN_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(robin_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ROBIN_HOME is a named profile.

    For ``~/.robin/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        robin_home: Optional explicit ROBIN_HOME path. Defaults to the current
            ``get_robin_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from robin_constants import get_default_robin_root
    home = Path(robin_home or str(get_robin_home())).resolve()
    default = get_default_robin_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ROBIN_HOME.

    Default ``~/.robin`` returns ``robin-gateway`` (backward compatible).
    Profile ``~/.robin/profiles/coder`` returns ``robin-gateway-coder``.
    Any other ROBIN_HOME appends a short hash for uniqueness.
    """
...
# robin-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (robin-gateway-*.service) and unrelated third-party
# "robin" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("robin.service",)
...
def _find_legacy_robin_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``robin.service`` before the rename to
    ``robin-gateway.service``). When both a legacy unit and the current
    ``robin-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``robin-gateway-coder.service`` and unrelated third-party
      ``robin-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``robin.service`` running an unrelated
...
def has_legacy_robin_units() -> bool:
    """Return True when any legacy Robin gateway unit files exist."""
    return bool(_find_legacy_robin_units())
...
def print_legacy_unit_warning() -> None:
    ...
    """
    legacy = _find_legacy_robin_units()
    if not legacy:
        return
    ...
    print_info("  These run alongside the current robin-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    robin gateway migrate-legacy")


def remove_legacy_robin_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Robin gateway unit files.

    Iterates over whatever ``_find_legacy_robin_units()`` returns — which is
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: robin gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo robin gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    robin gateway uninstall")
    print_info("    sudo robin gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Robin can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo robin gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo robin gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo robin gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.robin`` → ``ai.robin.gateway.plist`` (backward compatible).
    Profile ``~/.robin/profiles/coder`` → ``ai.robin.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.robin.gateway-{suffix}" if suffix else "ai.robin.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.robin/robin-agent  -> /home/alice/.robin/robin-agent
      /opt/robin                 -> /opt/robin  (kept as-is)
...
def _robin_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ROBIN_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_robin_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.robin                    → /home/alice/.robin
      /root/.robin/profiles/coder     → /home/alice/.robin/profiles/coder
      /opt/custom-robin               → /opt/custom-robin  (kept as-is)
    """
    current_robin = get_robin_home().resolve()
    current_default = (Path.home() / ".robin").resolve()
    target_default = Path(target_home_dir) / ".robin"

    # Default ~/.robin → remap to target user's default
    if current_robin == current_default:
        return str(target_default)

    # Profile or subdir of ~/.robin → preserve the relative structure
    try:
        relative = current_robin.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.robin) — keep as-is
        return str(current_robin)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        robin_home = _robin_home_for_target_user(home_dir)
        profile_arg = _profile_arg(robin_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m robin_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="ROBIN_HOME={robin_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    robin_home = str(get_robin_home().resolve())
    profile_arg = _profile_arg(robin_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Robin install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (robin.service from pre-rename installs)
    # before installing the new robin-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}robin gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}robin gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `robin gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}robin gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}robin gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}robin gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}robin gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.robin.gateway-{suffix}" if suffix else "ai.robin.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Robin install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  robin gateway status             # Check status")
    from robin_constants import display_robin_home as _dhh
...
def launchd_stop():
    ...
    # `robin gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_robin_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
    ...
    },
    {
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Robin will use (e.g., robin@gmail.com)."},
...
def _all_platforms() -> list[dict]:
    ...
    ``robin setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.robin/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_robin_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'robin pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: robin pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'robin gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from robin_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from robin_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print_info("  3. Robin will store the returned account_id/token in ~/.robin/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `robin gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_robin_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `robin pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `robin pairing approve`.")
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``robin_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from robin_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.robin/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.robin/.env: {', '.join(required)}")
    else:
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: robin gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: robin gateway install")
                else:
                    print_info("  You can install later: robin gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo robin gateway install --system")
                    print_info("  Or run in foreground:  robin gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: robin gateway run")
                print_info("  For persistence:   tmux new -s robin 'robin gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: robin gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup robin gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: robin gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'robin gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: robin gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: robin gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s robin 'robin gateway run'")
                print()
            ...
            print()
            print("  robin gateway run                              # direct foreground")
            print("  tmux new -s robin 'robin gateway run'         # persistent via tmux")
            print("  nohup robin gateway run > ~/.robin/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: robin gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: robin gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: robin gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: robin gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: robin gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  robin gateway install")
                    print("  sudo robin gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  robin gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup robin gateway run > ~/.robin/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s robin 'robin gateway run'         # persistent via tmux")
                    print("  nohup robin gateway run > ~/.robin/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  robin gateway install  # Install as user service")
                    print("  sudo robin gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy Robin gateway unit files from
        # pre-rename installs (e.g. robin.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_robin_units(interactive=not yes, dry_run=dry_run)