...
Usage:
    robin                     # Interactive chat (default)
    robin chat                # Interactive chat
    robin gateway             # Run gateway in foreground
    robin gateway start       # Start gateway as service
    robin gateway stop        # Stop gateway service
    robin gateway status      # Show gateway status
    robin gateway install     # Install gateway service
    robin gateway uninstall   # Uninstall gateway service
    robin setup               # Interactive setup wizard
    robin logout              # Clear stored authentication
    robin status              # Show status of all components
    robin cron                # Manage cron jobs
    robin cron list           # List cron jobs
    robin cron status         # Check if cron scheduler is running
    robin doctor              # Check configuration and dependencies
    robin honcho setup                    # Configure Honcho AI memory integration
    robin honcho status                   # Show Honcho config and connection status
    robin honcho sessions                 # List directory → session name mappings
    robin honcho map <name>               # Map current directory to a session name
    robin honcho peer                     # Show peer names and dialectic settings
    robin honcho peer --user NAME         # Set user peer name
    robin honcho peer --ai NAME           # Set AI peer name
    robin honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    robin honcho mode                     # Show current memory mode
    robin honcho mode [hybrid|honcho|local]  # Set memory mode
    robin honcho tokens                   # Show token budget settings
    robin honcho tokens --context N       # Set session.context() token cap
    robin honcho tokens --dialectic N     # Set dialectic result char cap
    robin honcho identity                 # Show AI peer identity representation
    robin honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    robin honcho migrate                  # Step-by-step migration guide: OpenClaw native → Robin + Honcho
    robin version             Show version
    robin update              Update to latest version
    robin uninstall           Uninstall Robin Agent
    robin acp                 Run as an ACP server for editor integration
    robin sessions browse     Interactive session picker with search

    robin claw migrate --dry-run  # Preview migration without changes
"""
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ROBIN_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (robin tools, robin setup, robin model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'robin {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any robin module import.
#
# Many modules cache ROBIN_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ROBIN_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.robin/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ROBIN_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 1.5 If ROBIN_HOME is already set and no explicit flag was given, trust it.
    # This lets child processes (relaunch, subprocess) inherit the parent's
    # profile choice without having to pass --profile again.
    if profile_name is None and os.environ.get("ROBIN_HOME"):
        return

    # 2. If no flag, check active_profile in the robin root
    if profile_name is None:
        try:
            from robin_constants import get_default_robin_root

            active_path = get_default_robin_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ROBIN_HOME
    if profile_name is not None:
        try:
            from robin_cli.profiles import resolve_profile_env

            robin_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent robin from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ROBIN_HOME"] = robin_home
...
# Load .env from ~/.robin/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from robin_cli.config import get_robin_home
from robin_cli.env_loader import load_robin_dotenv

load_robin_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ROBIN_REDACT_SECRETS env
# var BEFORE robin_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "ROBIN_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early
        _cfg_path = get_robin_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["ROBIN_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `robin` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from robin_logging import setup_logging as _setup_logging
...
try:
    from robin_cli.config import load_config as _load_config_early
    from robin_constants import apply_ipv4_preference as _apply_ipv4
...
from robin_cli import __version__, __release_date__
from robin_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from robin_cli.config import get_env_path, get_robin_home, load_config
    from robin_cli.auth import get_auth_status

    # Determine whether Robin itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from robin_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_robin_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from robin_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_robin_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from robin_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, robin_bin
        cli_args: the original CLI arguments (everything after 'robin')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    robin_bin = container_info["robin_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo robin {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo robin {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    print(f"  robin --tui --resume {target}")
    if title:
        print(f'  robin --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @robin/ink is missing or node_modules is behind package-lock.json.
    ...
    """
    ink = root / "node_modules" / "@robin" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: ROBIN_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("ROBIN_TUI_DIR")
    if env:
        p = Path(env)
...
def _tui_build_needed(tui_dir: Path) -> bool:
    if _robin_ink_bundle_stale(tui_dir):
        return True
...
def _robin_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "robin-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``ROBIN_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ROBIN_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    robin_home = os.environ.get("ROBIN_HOME") or str(Path.home() / ".robin")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ROBIN_HOME": robin_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(robin_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ROBIN_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ROBIN_NODE")
    ...
    # pre-built dist + node_modules (nix / full ROBIN_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("ROBIN_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("ROBIN_QUIET"):
    ...
    if tui_dev:
        if _robin_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/robin-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from robin_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="robin-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ROBIN_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ROBIN_PYTHON_SRC_ROOT"] = os.environ.get(
        "ROBIN_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ROBIN_PYTHON", sys.executable)
    env.setdefault("ROBIN_CWD", os.getcwd())
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")
    if model:
        env["ROBIN_MODEL"] = model
        env["ROBIN_INFERENCE_MODEL"] = model
    if provider:
        env["ROBIN_TUI_PROVIDER"] = provider
        env["ROBIN_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ROBIN_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    ...
    if resume_session_id:
        env["ROBIN_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("ROBIN_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'robin sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        ...
        print()
        print("  Run:  robin setup")
        print()

        from robin_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'robin setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from robin_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ROBIN_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.robin/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ROBIN_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ROBIN_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ROBIN_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from robin_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from robin_cli.config import get_env_value, save_env_value
    ...
    session_dir = get_robin_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: robin gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  robin gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Robin Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  robin gateway")
        ...
        print()
        print("  Or install as a service: robin gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'robin whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from robin_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``robin model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from robin_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from robin_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from robin_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ROBIN_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'robin model' for "
                "available providers, or run 'robin doctor' to diagnose config "
                "issues."
            )
    ...
    from robin_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from robin_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.robin/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.robin/.env if the active provider is not 'custom'.
    ...
    """
    from robin_cli.config import get_env_value, save_env_value, load_config
...
# `robin model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `robin model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from robin_cli.config import load_config, save_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``robin model`` flow, then route aux tasks to them here.
    """
    from robin_cli.config import load_config
    from robin_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from robin_cli.auth import _prompt_model_selection
    from robin_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from robin_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from robin_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from robin_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from robin_cli.iftv_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from robin_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from robin_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `robin auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from robin_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from robin_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from robin_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from robin_cli.auth import _save_model_choice, deactivate_provider
    from robin_cli.config import get_env_value, load_config, save_config
    ...
    from robin_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `robin model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from robin_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from robin_cli.config import get_env_value, save_env_value, load_config, save_config
    from robin_cli import azure_detect
    import getpass
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from robin_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from robin_cli.config import save_env_value, load_config, save_config
    from robin_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from robin_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from robin_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ROBIN_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so robin knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        from robin_cli.auth import AuthError
        from robin_cli.models import fetch_lmstudio_models
        ...
        from robin_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from robin_constants import display_robin_home as _dhh_fn
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from robin_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'robin model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    ...
    from robin_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from robin_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from robin_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from robin_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from robin_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from robin_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``robin slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: robin slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `robin slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from robin_cli.slack_cli import slack_manifest_command
...
def cmd_hooks(args):
    ...
    from robin_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from robin_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from robin_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from robin_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from robin_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from robin_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from robin_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from robin_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Robin Agent v{__version__} ({__release_date__})")
    ...
    try:
        from robin_cli.banner import check_for_updates
        from robin_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Robin Agent."""
    _require_tty("uninstall")
    from robin_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``robin update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``robin_cli/web_dist/`` (per vite.config.ts
    outDir: "../robin_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "robin_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``robin update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``robin web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (robin web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``robin dashboard`` processes other than ourselves.

    ``robin dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``robin update`` replaces files on disk, the running
    ...
    patterns = [
        "robin dashboard",
        "robin_cli.main dashboard",
        "robin_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "robin.*dashboard"`) keeps us consistent
            # with `robin_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``robin dashboard`` processes.

    Called at the end of ``robin update`` (default ``reason``) and also
    from ``robin dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    robin dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Robin Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/IfThenVoid/Robin/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="robin-update-")
        zip_path = os.path.join(tmp_dir, f"robin-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to robin-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"robin-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "robin-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes robin completely unrunnable (SyntaxError on import).
...
# =========================================================================
# Fork detection and upstream management for `robin update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/IfThenVoid/Robin.git",
    "git@github.com:IfThenVoid/Robin.git",
    "https://github.com/IfThenVoid/Robin",
    "git@github.com:IfThenVoid/Robin",
}
OFFICIAL_REPO_URL = "https://github.com/IfThenVoid/Robin.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_robin_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_robin_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from IfThenVoid/Robin.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/IfThenVoid/Robin.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/IfThenVoid/Robin.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``robin update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from robin_constants import get_default_robin_root

    default_home = get_default_robin_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "robin-agent[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``robin update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.robin/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``robin update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``robin update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.robin/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``robin update --gateway``) the update is already
    ...
    try:
        ...
        # robin_cli.config.get_robin_home to simulate setup failure.
        from robin_cli.config import get_robin_home as _get_robin_home

        logs_dir = _get_robin_home() / "logs"
...
def _cmd_update_check():
    """Implement ``robin update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``robin update`` without requiring a reinstall.  The
    ...
    ``robin`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/robin.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v robin'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/robin, code at /usr/local/lib/robin-agent).
    fhs_link = Path("/usr/local/bin/robin")
    ...
    path_comment = (
        "# Robin Agent — ensure /usr/local/bin is on PATH "
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ROBIN_HOME before running the update.
    ...
    ROBIN_HOME directories.  The ``--backup`` flag on ``robin update``
    ...
    try:
        from robin_cli.backup import create_pre_update_backup
    except Exception as exc:
    ...
    # Render path using display_robin_home so the user sees ~/.robin/...
    try:
        from robin_constants import get_robin_home, display_robin_home
        home = get_robin_home()
        try:
            display_path = f"{display_robin_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  robin import {out_path}")
...
def cmd_update(args):
    """Update Robin Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from robin_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Robin Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_robin_home added to robin_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload robin_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_robin_home() added since the last release.
        try:
            import importlib
            import robin_constants as _hc
        ...
        try:
            from robin_cli.profiles import (
                list_profiles,
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'robin config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: robin config migrate")
            else:
                print()
                print("Skipped. Run 'robin config migrate' later to configure.")
        else:
        ...
        # When running as ``robin update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_robin_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all robin-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. robin-gateway.service or robin-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: robin gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print(
                            "    (or: robin -p <profile> gateway run  for each profile)"
                        )
        ...
        # When both robin.service (from a pre-rename install) and the
        # current robin-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `robin update` surfaces the issue until the user migrates.
        try:
            from robin_cli.gateway import (
                has_legacy_robin_units,
                _find_legacy_robin_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_robin_units():
                print()
                print("⚠ Legacy Robin gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_robin_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (robin.service) fight the current")
                print("  robin-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    robin gateway migrate-legacy")
                print()
        ...
        print("  robin model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``robin -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from robin_constants import display_robin_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `robin profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_robin_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → robin -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.robin)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  robin profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     robin -p {name} chat")
                else:
        ...
        from robin_cli.profiles import profile_exists
        ...
        from robin_cli.profiles import rename_profile
        ...
        from robin_cli.profiles import export_profile
        ...
        from robin_cli.profiles import import_profile
...
def _report_dashboard_status() -> int:
    """Print ``robin dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``robin dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No robin dashboard processes running.")
        return 0

    print(f"{len(pids)} robin dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No robin dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `robin update`.
    ...
    if "ROBIN_WEB_DIST" not in os.environ:
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from robin_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("ROBIN_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from robin_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from robin_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for robin CLI."""
    from robin_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: robin-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from robin_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://github.com/IfThenVoid/Robin/tree/main/website/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `robin model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy robin.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Robin gateway unit files "
            "(e.g. robin.service) left over from older installs. Profile "
            "units (robin-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Robin Agent with an interactive wizard. "
        "Run a specific section: robin setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        help="(Default on existing installs.) Re-run the full wizard, "
             ...
             "compatibility — a bare 'robin setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_manifest.add_argument(
        ...
        help="Write manifest to a file instead of stdout. With no PATH "
             "writes to $ROBIN_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: robin-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate Robin with Spotify via PKCE")
    auth_spotify.add_argument("spotify_action", nargs="?", choices=["login", "status", "logout"], default="login")
    auth_spotify.add_argument("--client-id", help="Spotify app client_id (or set ROBIN_SPOTIFY_CLIENT_ID)")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Robin Agent components",
    )
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.robin/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.robin/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Robin Agent setup",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Robin Agent. Use 'robin debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    robin debug share              Upload debug report and print URL
    robin debug share --lines 500  Include more log lines
    robin debug share --expire 30  Keep paste for 30 days
    robin debug share --local      Print report locally (no upload)
    robin debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'robin debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        ...
        "skills, sessions, and data (excludes the robin-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/robin-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Robin Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from robin_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.robin/skills/.bundled_manifest) "
            "so future 'robin update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from robin_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from robin_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/robin-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `robin plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from robin_cli.plugins_cmd import plugins_command
    ...
    try:
        from robin_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_robin_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_robin_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_robin_home()}/memories/\n")
        else:
            from robin_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'robin tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # robin tools disable <name...> [--platform cli]
    ...
    # robin tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from robin_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from robin_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'robin mcp add' to connect to a new server, or\n"
            "'robin mcp serve' to expose Robin conversations over MCP."
        ),
    )
    ...
    def cmd_mcp(args):
        from robin_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch robin --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from robin_cli.relaunch import relaunch
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.robin/ (by default a "
        "single restore-point archive is written to ~/.robin/backups/ "
        "before apply; restorable with 'robin import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from robin_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Robin Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Robin Agent",
        description="Remove Robin Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Robin Agent as an ACP (Agent Client Protocol) server",
        description="Start Robin Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Robin Agent as an ACP server."""
        try:
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Robin Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `robin --tui` via PTY/WebSocket). "
            "Alternatively set ROBIN_DASHBOARD_TUI=1."
        ),
    )
    ...
    # for `robin dashboard` cmdlines and SIGTERM them directly — the same
    # path `robin update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running robin dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running robin dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    robin logs                    Show last 50 lines of agent.log
    robin logs -f                 Follow agent.log in real time
    robin logs errors             Show last 50 lines of errors.log
    robin logs gateway -n 100     Show last 100 lines of gateway.log
    robin logs --level WARNING    Only show WARNING and above
    robin logs --session abc123   Filter by session ID
    robin logs --component tools  Only show tool-related lines
    robin logs --since 1h         Lines from the last hour
    robin logs --since 30m -f     Follow, starting from 30 min ago
    robin logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``robin -c Pokemon Agent Dev`` → ``robin -c 'Pokemon Agent Dev'``
    ...
    from robin_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'robin -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (robin hooks list, cron
    ...
    if getattr(args, "oneshot", None):
        from robin_cli.oneshot import run_oneshot
