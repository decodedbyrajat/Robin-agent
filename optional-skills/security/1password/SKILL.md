...
version: 1.0.0
author: arceus77-7, enhanced by Robin Agent
license: MIT
metadata:
  robin:
...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.robin/.env` (the skill will prompt for this on first load).
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/robin-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/robin-op.sock"
