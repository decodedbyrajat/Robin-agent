...
into Robin Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.robin/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.robin/tts/.",
    },
    "discord-settings": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'robin cron'.",
    },
    "hooks-config": {
...
# like ``openclaw`` → ``robin`` (so filesystem paths like ``~/.openclaw``
# become ``~/.robin`` — the real Robin home — not the broken ``~/.Robin``).
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Robin`` but maps ``openclaw`` → ``robin`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.robin/config.yaml`` (the real Robin home) instead of the broken
    ``~/.Robin/config.yaml``.
    """
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-robin/apply.ts "blocked by earlier
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-robin/plan.ts warnings[].
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in ("file", "exec"):
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: robin config set",
                        )
                    continue
        ...
        for oc_key, robin_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and robin_key not in secret_additions:
                secret_additions[robin_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, robin_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and robin_key not in secret_additions:
                        secret_additions[robin_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        robin_config = load_yaml_file(destination)
        current_model = robin_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = robin_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                robin_config["model"] = {"default": model_str}
            dump_yaml_file(destination, robin_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        robin_config = load_yaml_file(destination)
        existing_tts = robin_config.get("tts", {})
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        robin_cfg_path = self.target_root / "config.yaml"
        robin_cfg = load_yaml_file(robin_cfg_path)
        existing_mcp = robin_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            robin_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                robin_srv["command"] = srv["command"]
                if srv.get("args"):
                    robin_srv["args"] = srv["args"]
                if srv.get("env"):
                    robin_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    robin_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                robin_srv["url"] = srv["url"]
                if srv.get("headers"):
                    robin_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    robin_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                robin_srv["enabled"] = False
            if srv.get("timeout"):
                robin_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                robin_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                robin_srv["tools"] = {}
                if tools_cfg.get("include"):
                    robin_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    robin_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(robin_cfg_path)
            robin_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(robin_cfg_path, robin_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'robin cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = robin_cfg.get("agent") or {}
        ...
        if compaction:
            compression = robin_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'robin gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("ROBIN_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        robin_cfg_path = self.target_root / "config.yaml"
        robin_cfg = load_yaml_file(robin_cfg_path)
        sr = robin_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        robin_cfg_path = self.target_root / "config.yaml"
        robin_cfg = load_yaml_file(robin_cfg_path)
        custom_providers = robin_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(robin_cfg_path)
            robin_cfg["custom_providers"] = custom_providers
            dump_yaml_file(robin_cfg_path, robin_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            robin_cfg_path = self.target_root / "config.yaml"
            robin_cfg = load_yaml_file(robin_cfg_path)
            discord_robin = robin_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_robin["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_robin["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                robin_cfg["discord"] = discord_robin
                dump_yaml_file(robin_cfg_path, robin_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        robin_cfg_path = self.target_root / "config.yaml"
        robin_cfg = load_yaml_file(robin_cfg_path)
        browser_robin = robin_cfg.get("browser") or {}
        changed = False

        # Map fields that have Robin equivalents
        if browser.get("cdpUrl"):
            browser_robin["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_robin["headless"] = browser["headless"]
            changed = True

        if changed:
            robin_cfg["browser"] = browser_robin
            if self.execute:
                self.maybe_backup(robin_cfg_path)
                dump_yaml_file(robin_cfg_path, robin_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            robin_mode = mode_map.get(mode, "manual")
            robin_cfg.setdefault("approvals", {})["mode"] = robin_mode
            if self.execute:
                self.maybe_backup(robin_cfg_path)
                dump_yaml_file(robin_cfg_path, robin_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{robin_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "",
            "**Strongly recommended:** Run `robin claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `robin claw cleanup` to fix it.",
            "",
            "## Robin-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `robin claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `robin setup` to configure any remaining settings",
            "- Run `robin mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `robin cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `robin cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `robin gateway install` if you need the gateway service",
            "- Review `~/.robin/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Robin Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=str(Path.home() / ".robin"), help="Robin home directory")
    parser.add_argument(
        "--workspace-target",
...
def main() -> int:
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.robin/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.robin/config.yaml")
        print("    2. Run: robin mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: robin cron")
