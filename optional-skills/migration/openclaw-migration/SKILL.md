...
description: Migrate a user's OpenClaw customization footprint into Robin Agent. Imports Robin-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Robin Agent (IfThenVoid)
license: MIT
metadata:
  robin:
    tags: [Migration, OpenClaw, Robin, Memory, Persona, Import]
    related_skills: [robin-agent]
---

# OpenClaw -> Robin Migration

Use this skill when a user wants to move their OpenClaw setup into Robin Agent with minimal manual cleanup.
...
```bash
robin claw migrate              # Full interactive migration
robin claw migrate --dry-run    # Preview what would be migrated
robin claw migrate --preset user-data   # Migrate without secrets
robin claw migrate --overwrite  # Overwrite existing conflicts
robin claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `robin setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_robin.py` to:
...
- migrate Robin-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.robin/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Robin workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.robin/tts/`
...
- `scripts/openclaw_to_robin.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.robin/skills/migration/openclaw-migration/scripts/openclaw_to_robin.py`

Do not guess a shorter path like `~/.robin/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.robin/skills/migration/openclaw-migration/`.
...
```bash
python3 ~/.robin/skills/migration/openclaw-migration/scripts/openclaw_to_robin.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.robin/skills/migration/openclaw-migration/scripts/openclaw_to_robin.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.robin/skills/migration/openclaw-migration/scripts/openclaw_to_robin.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.robin/skills/migration/openclaw-migration/scripts/openclaw_to_robin.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.robin/skills/migration/openclaw-migration/scripts/openclaw_to_robin.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.robin/skills/migration/openclaw-migration/scripts/openclaw_to_robin.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
- OpenClaw skills available under `~/.robin/skills/openclaw-imports/`
