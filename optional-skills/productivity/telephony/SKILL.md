...
- save provider credentials into `~/.robin/.env`
...
```bash
SCRIPT="$(find ~/.robin/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
robin skills search telephony
robin skills install official/productivity/telephony
```
...
3. Buy it and save it into `~/.robin/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.robin/.env`
