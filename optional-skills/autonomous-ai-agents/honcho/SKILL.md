...
See: https://docs.honcho.dev/v3/guides/integrations/robin#running-honcho-locally-with-robin

### Verify

```bash
robin honcho status    # shows resolved config, connection test, peer info
```
...
Manual override: `robin honcho map my-project-name`
...
```bash
robin profile create coder --clone
# creates host block robin.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `robin.coder` host block in `honcho.json`
...
```bash
robin honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Config file: `$ROBIN_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `robin honcho setup`. Ensure `memory.provider: honcho` is in `~/.robin/config.yaml`.

### Memory not persisting across sessions
Check `robin honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `robin profile create <name> --clone`. For existing profiles: `robin honcho sync`.
...
| Command | Description |
|---------|-------------|
| `robin honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `robin honcho status` | Show resolved config, connection test, peer info for active profile |
| `robin honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `robin honcho disable` | Disable Honcho for the active profile |
| `robin honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `robin honcho peers` | Show peer identities across all profiles |
| `robin honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `robin honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `robin honcho sessions` | List known directory-to-session-name mappings |
| `robin honcho map <name>` | Map current working directory to a Honcho session name |
| `robin honcho identity` | Seed AI peer identity or show both peer representations |
| `robin honcho sync` | Create host blocks for all Robin profiles that don't have one yet |
| `robin honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Robin + Honcho |
| `robin memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `robin memory status` | Show active memory provider and config |
| `robin memory off` | Disable external memory provider |
