...
The PID file lives at ``{ROBIN_HOME}/gateway.pid``.  ROBIN_HOME defaults to
``~/.robin`` but can be overridden via the environment variable.  This means
separate ROBIN_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ROBIN_HOME."""
    home = get_robin_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ROBIN_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "robin" / _LOCKS_DIRNAME
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ROBIN_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``robin.service`` + ``robin-
# gateway.service``).
#
