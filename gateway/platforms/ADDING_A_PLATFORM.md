...
Create a plugin directory in `~/.robin/plugins/` with a `PLUGIN.yaml` and
...
requires **zero changes to core Robin code**.
...
And add it to the `robin-gateway` composite:

```python
"robin-gateway": {
    "includes": [..., "robin-your-platform"]
}
```
...
## 12. Status Display (`robin_cli/status.py`)
...
## 13. Gateway Setup Wizard (`robin_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ robin_cli/ toolsets.py \
  --include="*.py" -l | sort -u
