...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Robin-Session-Id header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists robin-agent as an available model
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to robin-agent
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_robin_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through robin-agent's AIAgent.
    """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``robin sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the robin-api-server default.
        """
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "robin-agent"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return robin-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "robin.api_server.capabilities",
            "platform": "robin-agent",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``robin.tool.progress`` with ``status: running``.
        ...
        return web.json_response(response_data, headers={"X-Robin-Session-Id": session_id})
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: robin.tool.progress`` SSE event so
